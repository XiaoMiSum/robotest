...
const selectedRole = ref<{ id: string; isSystem: boolean; type: string } | null>(null)
const activeTab = ref<'permissions' | 'users'>('permissions')

function handleSelect(node: { id: string; isSystem: boolean; type: string }) {
      ...
      <el-card shadow="never" class="role-page__main">
        ...
        <el-tabs v-else v-model="activeTab">
          <el-tab-pane label="权限配置" name="permissions">
            <PermissionTable :role-id="selectedRole.id" :is-system="selectedRole.isSystem" :role-type="selectedRole.type" />
          </el-tab-pane>
