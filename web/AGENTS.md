...
- 脑图：自研 SVG/Canvas + Yjs CRDT 协同（详见 `docs/00-spec/05-api.md` 第 9 节）

> 详细分层职责参见 `docs/00-spec/03-frontend.md`；页面分区滚动与滚动条视觉隐藏参见 `docs/00-spec/13-scroll-container.md`。
...
- 后端返回的时间为 UTC+0 无时区标识字符串，展示必须走 `utils/format.ts` 的 `formatDateTime` / `formatDate` 转本地时区，禁止直接 `new Date()` 或直接插值（详见 `docs/00-spec/03-frontend.md` 第 9 节）
