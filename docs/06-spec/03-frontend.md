...
declare module 'vue-router' {
  interface RouteMeta {
    ...
    title?: string          // 同时用于浏览器标签页标题（router.afterEach → document.title）
    mode?: 'admin' | 'workspace' | 'project' | 'none' // 顶栏导航模式，nav store 从当前路由派生
    menu?: {                // 菜单项：缺省即页面不进任何菜单（详情/创建页），路由表即菜单注册表
      label: string
      icon: string
      order: number         // 排序权重（小者靠前），不依赖路由注册顺序
      section?: string      // 管理端侧边栏分组标题；缺省 = 置顶不分组
      permission?: string
...
> 顶部动态菜单（`nav` store `dynamicMenuItems`）与管理端侧边栏（`adminSidebarSections`）均从路由注册表按 `meta.menu` + `meta.mode` + 权限过滤派生，禁止在 store/布局中手写第二份菜单路径。
