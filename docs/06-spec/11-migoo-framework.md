...
本文定义项目使用 `migoo-spring-boot-starter` 时的组件选择、版本管理、项目适配边界和升级验证要求。

本文只规定项目如何选择、配置、验证和约束框架能力，不复制完整的组件 API 手册。API 速查见 [`17-migoo-api-reference.md`](17-migoo-api-reference.md)；官方 API、配置项和示例以官方文档及当前锁定版本为准。
...
项目复用框架的 `Result`、`ErrorCode`、业务异常工具、分页和通用工具，不创建平行的响应包装或异常体系。
...
Web 组件负责全局异常、响应处理、TraceId、请求上下文、i18n、CORS 和请求体缓存等框架级能力。项目只补充业务错误码、DTO 校验、安全响应头、审计脱敏和 SSE/文件/实时通信等特殊响应适配。

生产环境不得依赖宽松默认配置，尤其是 CORS、请求体缓存和敏感日志配置。

### 4.3 MyBatis

项目实体使用框架公共基类，Mapper 使用 `BaseMapperX` 或其项目扩展；复杂和可复用查询下沉 Mapper，简单动态过滤可以由 Service 组合。

项目覆盖项：

- `BaseUuidDO` 的 UUID 策略由框架默认实现决定，不在项目规范中强制 UUID v4/v7；
...
MapStruct 不属于 migoo Starter 的通用 API。项目统一要求：
...
WebSocket 组件提供连接管理、Token 认证、会话、房间和分布式广播能力。项目接入要求：
...
通用协议见 `15-realtime-protocol.md`，项目级接入设计见 `docs/04-detailed-design/78-realtime-websocket.md`；业务事件不在本文定义。

### 4.7 Redis

Redis 能力用于缓存、限流、会话、短期 Ticket 和分布式协作。项目必须集中登记 Key 命名、TTL、值类型、失效/重建策略、锁保护、敏感值处理和 Redis 不可用时的降级行为。

### 4.8 MQ

项目按可靠性选择 Stream 或 Pub/Sub：Stream 用于可靠投递、ACK、重试、死信和幂等消费；Pub/Sub 用于广播，不作为唯一业务事实源。消息模型、Channel、幂等键、重试和告警必须集中登记。
