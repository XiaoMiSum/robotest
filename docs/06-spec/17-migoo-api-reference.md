...
本文是项目使用 migoo 框架时的 API 速查摘要，帮助开发者定位当前版本可用的组件入口、典型类和适配边界。

本文不是官方 API 手册，也不替代锁定版本的官方文档。组件页面 URL、版本和发布说明见 [`11-migoo-framework.md`](11-migoo-framework.md) 2. 官方文档与本文不一致时，以官方文档和实际依赖版本为准，并在项目规范中记录差异。

## 2. 使用边界

- 本文只说明“框架提供什么入口”和“项目如何适配”；
- 项目的响应字段、错误码、分页参数、权限和业务异常规则分别以 `05-api.md`、`10-security.md` 和 `04-backend.md` 为准；
...
| 错误响应 | `Result.error(ErrorCode)` | 错误码登记在项目 `ErrorCodeConstants` |
| 业务异常 | `ServiceExceptionUtil.get(ErrorCode, args...)` | 只用于项目业务错误，不创建平行异常体系 |
| 分页请求 | `PageParam` | 项目统一 `pageNo/pageSize` |
| 分页结果 | `PageResult<T>` | 对外使用 `list/total` |
| 参数校验 | migoo validation 注解 | 组合项目 DTO 校验和安全规则 |
...
`PageParam` 和 `PageResult` 是框架能力入口；项目默认值、最大值、游标分页和响应字段由 API 规范统一覆盖。
...
| 能力 | 典型入口 | 项目适配 |
| --- | --- | --- |
| 全局异常 | 框架 Web 异常处理器 | 统一转换为项目 `Result` 和 `ErrorCodeConstants` |
...
项目不在 Controller 中重复实现全局异常、响应包装或跨域基础设施；业务 DTO 校验、业务审计和特殊响应适配仍由项目层负责。
...
| Mapper 基类 | `BaseMapperX<T>` | 提供项目使用的查询、分页和批量方法 |
...
查询结果只用于校验，不得作为包含全部字段的 `updateById` 载体。更新载体只设置本次请求实际变更的字段；需要显式写入 `null` 时使用 `LambdaUpdateWrapperX`。该规则是项目约束，不是框架默认行为。
...
MapStruct 不是 migoo Starter 的通用 API，项目约定如下：

- 转换器统一放在 `server/src/main/java/**/model/convert/`；
...
框架认证成功不等于业务授权成功。管理端、工作空间、项目和 WebSocket 资源均必须在服务端校验角色与归属关系。
...
通用消息信封、错误码、生命周期和安全要求见 `15-realtime-protocol.md`；项目级 Ticket、Origin、前端适配器和实现差距见 `docs/04-detailed-design/78-realtime-websocket.md`。业务事件不得在本文档中定义。
...
5. 更新本参考、项目集成规范、部署 Runbook 和回滚说明。

## 12. 参考

- 项目集成策略：[`11-migoo-framework.md`](11-migoo-framework.md)
