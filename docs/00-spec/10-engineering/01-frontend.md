...
请求服务必须遵守 `docs/00-spec/20-contracts/01-api.md`：
...
- 页面和组件优先使用 Element Plus 组件默认样式及其配置属性、插槽和 CSS 变量，不得用自定义 CSS 重写组件内部结构；默认能力不足时中断实现并由开发者确认（UI-DS-09）。
...
当前跨端时间契约由 `docs/00-spec/20-contracts/01-api.md` 定义。前端时间处理必须：

- 所有 API `date-time` 字段（包括过期时间）按 UTC `Z` 时间解析，并统一转换为浏览器所在时区展示。
- 完整时间、日期和相对时间使用统一时间工具；禁止页面自行解析或拼接时间字符串。
- 纯日期字段按日历日期展示，不做时区转换。
- 时间比较、排序和倒计时必须使用统一解析结果，禁止直接对原始字符串调用 `new Date()` 或插值显示。
- 对后端仍以 `LocalDateTime` 接收的既有请求字段，提交无时区墙钟值，不得在前端预先转换为 `Z` 后再交给服务端持久化适配层。
- 测试必须覆盖 UTC `Z`、带 offset、无时区兼容输入、非法输入和浏览器时区变化。
...
- 变更覆盖率目标遵循 `docs/00-spec/30-quality-delivery/01-quality.md`，不得通过删除测试或降低阈值绕过门禁。
