...
| DOC-005 | P0 | 同步通用 WebSocket 详细设计 | DEC-007 | 明确连接、房间/主题、通用帧、错误、生命周期和鉴权流程；业务事件由详细设计定义 | 已完成 |
...
| 编号 | 优先级 | 任务 | 依赖 | 完成标准 | 状态 |
| --- | --- | --- | --- | --- | --- |
| CODE-001 | P0 | 将 MapStruct 转换器迁移到 `model/convert/` | DEC-008 | 所有转换器路径统一，编译和转换测试通过 | 进行中：实现与测试已完成，待独立提交 |
...
| 编号 | 优先级 | 任务 | 依赖 | 完成标准 | 状态 |
| --- | --- | --- | --- | --- | --- |
| SEC-001 | P0 | 移除生产可预测默认密钥并实现 fail-fast | — | 生产缺少密钥时启动失败，已暴露密钥完成轮换 | 进行中 |
| SEC-002 | P0 | 为 `/api/admin/**` 增加服务端角色/权限校验 | — | 只有工作空间角色不能访问管理端 API | 待实施 |
| SEC-003 | P0 | 将 workspace/project 上下文校验改为 fail-closed | 各业务详细设计边界 | 无成员、无角色、项目越界全部拒绝 | 待实施 |
...
- CI 覆盖率硬门禁；
- OpenAPI 契约基线和漂移阻断；
- 生产密钥 fail-fast 已完成；仍需持续轮换已暴露的历史密钥；
