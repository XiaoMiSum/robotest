...
- 通用视觉、状态、响应式和可访问性见 `docs/00-spec/50-ui/01-frontend-design.md`。
- 滚动区域和滚动拥有者见 `docs/00-spec/50-ui/03-scroll-container.md`。
...
### 6.6 UI-PAGE-11：列表型页面必须显式呈现请求错误

列表型页面或承担列表数据加载的页面级 composable，必须为首次加载、分页、筛选、刷新和重试建立可见的错误边界：

- 请求失败必须由页面或页面级 composable 捕获，并通过统一消息提示机制向用户展示可理解的错误信息；不能只依赖全局请求拦截器，也不能静默吞掉异常；
- 优先使用后端返回的可展示错误消息；消息缺失或不适合展示时，使用当前页面场景的兜底文案，不暴露堆栈、内部标识或敏感信息；
- 错误提示必须有明确的唯一责任层，避免拦截器和页面重复弹出同一错误；
- 只有当前有效请求可以更新状态并提示错误，已被新请求取代的过期响应不得显示误导性消息；
- 错误态与空态分离，页面应保留刷新或重试入口，并在加载结束后正确解除加载状态；
- 页面级错误提示、状态清理和重试流程必须有测试覆盖。

该规则只约束列表数据流的错误可见性和恢复边界，不规定具体消息组件、文案、路由或业务字段。
...
| UI-PAGE-11 | 列表错误可见性 | 列表加载、分页、筛选和刷新失败由责任层提示，且具备恢复入口。 |
...
- 前端通用设计：`docs/00-spec/50-ui/01-frontend-design.md`
