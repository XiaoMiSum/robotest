...
> 本分册由《》按功能模块拆分而来。前言、引言、数据设计与公共约定见总览分册 `24-interface-ui-overview.md`；原章节编号保持不变，分册-章节对照见总览分册。

---

## 3. 接口定义编辑器

> 当前版本按 MeterSphere 参考重设计。交互原型见 `docs/交互设计/接口编辑器原型.html`。
...
**刷新/直链恢复**：以 `?tab=interfaces&interfaceId=<id>`（编辑）或 `?tab=interfaces&action=create`（新建，可附加 `&moduleId=`）恢复并打开对应编辑器 Tab，约定同 `docs/交互设计/README.md` 场景编辑器。
...
请求体编辑器**对齐快速调试**（见 `docs/交互设计/README.md`），类型互斥为 **none / x-www-form-urlencoded / raw**，切换类型时**保留各类型已填内容**，切回后恢复：
...
接口定义在请求 Tab 之后提供 **接口级配置 Tab**，配置随接口定义整体保存、回显；本期**仅定义存储**，不带入请求执行链路（执行消费留待测试场景模块，边界见 `docs/详细设计/README.md`）。
...
响应区**独立于请求信息区**（上下分区），仅**单条响应**，存储为接口的 `responseExample`，展示形态与 `docs/交互设计/README.md` 响应查看器一致：
...
| 执行 | 点击 [▶ 执行] | 将当前请求配置发送调试，结果展示于响应区「执行结果」态（复用 `docs/交互设计/README.md` 执行链路与结果查看器） |
