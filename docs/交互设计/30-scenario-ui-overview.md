...
> 环境管理交互设计见 `docs/交互设计/README.md`；导航框架与色彩体系见 `docs/交互设计/全局导航与菜单交互系统设计.md`、`docs/交互设计/视觉设计.md`。
...
- **环境**为执行提供变量、Base URL、默认配置、数据源与全局处理器；场景执行时必须选择目标环境（可配置默认环境）。环境管理交互见 `docs/交互设计/README.md`。
...
- 弹窗类型汇总（表单 / 选择器 / 确认 / 执行 / 帮助 / 提示条）见 `docs/交互设计/README.md` 4.1；
