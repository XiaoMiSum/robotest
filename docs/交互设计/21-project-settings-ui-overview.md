# 软件测试平台——（总览分册）

**文档版本**：V1.0  
**日期**：2026-09-23  
**状态**：起草中

> 本分册为《》按功能模块拆分后的总览分册，承载前言、引言、数据设计与公共约定等非模块内容；各功能模块正文见本目录对应分册，原章节编号保持不变，分册-章节对照表见文末。

---

> 本文档为接口测试域交互设计分册：平台级「项目设置」框架规则的前端交互设计。环境管理、全局资产、函数管理的页面交互见各自分册；导航框架见 `docs/交互设计/全局导航与菜单交互系统设计.md` 3.5，色彩体系见 `docs/交互设计/视觉设计.md`。

---

## 1. 概述

### 1.1 模块范围

本文档覆盖接口测试业务域（承接 SRS 3.7 项目设置）的以下内容：

1. **项目设置框架交互**——平台级统一入口在各业务域侧边栏的呈现规则、域归属过滤、权限控制。

环境管理、全局资产、函数管理的页面交互分别见 `docs/交互设计/README.md`、`docs/交互设计/全局资产交互设计.md` 自身分册，本文档不重复描述。

### 1.2 菜单与路由

「项目设置」分组固定位于接口测试侧边栏末尾（完整侧边栏树见各分册 1.2），以分隔线与业务功能区分隔：

| 页面 | 路由 | 模式 | 权限 |
| ---- | ---- | ---- | ---- |
| 环境管理 | `/workspace/projects/settings/environments` | 项目模式 | 项目成员查看/使用；项目维护者维护 |
| 函数管理 | `/workspace/projects/settings/functions` | 项目模式 | 项目成员查看；项目维护者维护 |
| 全局资产 | `/workspace/projects/settings/assets` | 项目模式 | 项目成员查看/复制引入；项目维护者维护 |

> 上表路由为前端路由示意，与后端 API 路径相互独立。路由前缀 `/workspace/projects/settings/*` 为平台级统一约定，跨业务域一致。

### 1.3 权限边界

- 项目设置的查看为项目成员权限；维护（增删改）为项目维护者权限。
- 项目成员进入任何设置页均为只读态：维护控件禁用并提示「需项目维护者」。

---


## 4. 通用交互模式

弹窗类型、加载状态、错误处理等通用交互模式与《测试场景交互设计》第 6 章一致，本文档不重复描述：

- 弹窗类型（确认弹窗用于删除等危险操作确认）见 `docs/交互设计/README.md` 6.1；
- 加载状态（骨架屏、按钮 loading）见 6.2；
- 错误处理（业务错误码、乐观锁冲突 409）见 6.3。

---

**文档结束**

## 分册-章节对照表

| 分册 | 文件 | 覆盖章节 |
|---|---|---|
| 总览 | `21-project-settings-ui-overview.md` | 前言、1. 概述、4. 通用交互模式 |
| 设置框架 | `22-project-settings-ui-framework.md` | 2. 项目设置框架交互 |
| 函数管理页 | `23-project-settings-ui-function.md` | 3. 函数管理页 |
