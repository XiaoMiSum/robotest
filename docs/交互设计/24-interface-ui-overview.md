...
> 快速调试与导入功能见 `docs/交互设计/README.md`。
...
快速调试（SRS 3.1）与导入功能见 `docs/交互设计/README.md`。

接口定义是测试场景与 Mock 服务的数据底座，本域产出的接口资产被 `docs/交互设计/README.md`、`docs/交互设计/README.md`、`docs/交互设计/README.md`、`docs/交互设计/README.md` 引用。

### 1.2 侧边栏菜单与路由

项目模式顶部菜单在既有「功能测试」「缺陷管理」之外**新增「接口测试」入口**（增量见 `docs/交互设计/全局导航与菜单交互系统设计.md` 3.3）。接口测试页面内部采用侧边栏组织子模块：
...
- 变量引用统一 `${变量名}` 语法，支持内置函数（见 `docs/交互设计/README.md` 第 3 章引用帮助）。
- 变量解析优先级（低→高）：**内置函数 < 环境变量 < 场景参数 < 步骤级变量 < 步骤提取器变量 < 运行时覆盖**（见 `docs/详细设计/README.md` 4.1）。
- 快速调试（验证入口）与导入（资产来源）见 `docs/交互设计/README.md`。
...
本部分涉及的通用交互模式遵循 `docs/交互设计/README.md` 第 4 章的约定：
