...
- **权限**：同 3.1.4（创建人或项目管理权限成员），违规返回 2001。
...
- **校验**：`items` 非空且 ≤ 100 条；每项 `title` ≤ 200、`content` 长度 ≤ `requirementContentMaxLength`（同 3.1.3），超限返回 1001；`aiGenerated` 缺省 false（仅作展示标记，不影响业务规则，接受客户端透传）。
- **权限**：与 3.1.3 一致（创建人本人即可，项目管理权限成员亦可）；批量接口不做逐条创建人差异。
- **响应**：`{ "count": 5 }`（实际入库条数）。
