...
本文档对接口测试业务域的 **Mock 服务**进行详细设计，定义 Mock 定义、匹配规则、响应、调试、访问的数据结构、接口规范与业务逻辑，为开发实现提供完整依据。接口测试报告（报告查看、分享、清理）相关设计见《测试报告详细设计说明书》（`docs/详细设计/测试报告详细设计说明书.md`）。
...
- 《接口测试需求规格说明书》（`docs/需求/接口测试需求规格说明书.md`，3.3）
