...
> **全局环境（预留扩展）**：环境均为项目级（`scope = project`），界面与接口为全局环境保留扩展位（类型徽标「全局」样式与展示位），维护入口与切换逻辑待后续版本补充（见 `docs/交互设计/环境管理交互设计.md` 2.4）。
...
>
> 请求方法缺省 GET；响应超时固定 30000ms、连接超时固定 10000ms、跟随重定向开启、SSL 校验开启（`default_method`/`timeout_ms`/`connect_timeout_ms`/`follow_redirects`/`verify_ssl` 字段已移除，见 `docs/交互设计/环境管理交互设计.md` 2.3）。
