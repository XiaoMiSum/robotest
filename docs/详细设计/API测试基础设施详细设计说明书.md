...
- 《接口测试需求规格说明书》（`docs/需求/接口测试需求规格说明书.md`，3.8–3.12）
- 《概要设计说明书》（`docs/概要/概要设计说明书.md`，4.1–4.2）
...
3. **启用禁用**：每个验证器可独立启用/禁用（开关配置于基础信息，见 5.3 公共字段表）
...
切换类型为提取器时，展示提取器配置区。采用**平台自有数据结构**（启用/禁用开关配置于基础信息，见 5.3 公共字段表）：
