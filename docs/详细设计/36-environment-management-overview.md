...
测试场景编排（SRS 3.4、3.9–3.11）的相关设计见《测试场景详细设计说明书》（`docs/详细设计/README.md`）。

### 1.3 参考资料

- 《接口测试需求规格说明书》（`docs/需求/README.md`，3.5）
- 《概要设计说明书》（`docs/概要/概要设计说明书.md`，3.1）
- 《API 测试基础设施详细设计说明书》（`docs/详细设计/README.md`）
...
> **全局环境（预留扩展）**：环境均为项目级（`scope = project`），界面与接口为全局环境保留扩展位（类型徽标「全局」样式与展示位），维护入口与切换逻辑待后续版本补充（见 `docs/交互设计/README.md` 2.4）。
...
>
> 请求方法缺省 GET；响应超时固定 30000ms、连接超时固定 10000ms、跟随重定向开启、SSL 校验开启（`default_method`/`timeout_ms`/`connect_timeout_ms`/`follow_redirects`/`verify_ssl` 字段已移除，见 `docs/交互设计/README.md` 2.3）。
