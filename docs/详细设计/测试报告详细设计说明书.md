...
本文档对软件测试平台 V1.2 接口测试业务域的 **接口测试报告**进行详细设计，定义报告生成、分享、批量删除、状态同步、清理的业务逻辑与前端行为，为开发实现提供完整依据。报告的数据表（`api_report`）与报告接口（列表、详情、分享、删除）在《API 测试基础设施详细设计说明书》中定义，本文档不重复描述。Mock 服务相关设计见《Mock 服务详细设计说明书》（`docs/详细设计/Mock服务详细设计说明书.md`）。
