...
覆盖 SRS 3.4（缺陷智能分析与去重）与概要 4.5（向量检索机制）。用例向量（case_embedding）的**消费方**（遗漏测试点分析、回归子集推荐）见《AI 评审与测试计划辅助详细设计说明书》，其索引对象与写入时机在本文档定义（向量基建统一收口）。
...
**索引**：`uk_bug_embedding_bug_id` UNIQUE (bug_id) WHERE is_deleted = false，`idx_bug_embedding_project_id` (project_id)，
`idx_bug_embedding_hnsw` USING hnsw (embedding vector_cosine_ops)

#### 2.1.2 用例向量表（case_embedding）
...
**索引**：`uk_case_embedding_node_id` UNIQUE (node_id) WHERE is_deleted = false，`idx_case_embedding_project_id` (project_id)，
`idx_case_embedding_hnsw` USING hnsw (embedding vector_cosine_ops)

> 不冗余 `document_id`：现有消费方（遗漏分析、回归推荐）均为项目内过滤，无按文档过滤的检索；将来需要时可经 `test_case_node` JOIN 获取（文档删除时的向量批量清理同理），避免无消费方的冗余列与索引。
...
- 补偿扫描 SQL 按项目分组、每轮上限 200 条，避免长时间占用；扫描范围含用例节点（case_embedding 同链路）；hash 过期判定需对全量未关闭缺陷与 case 节点重算索引文本 hash，千级规模成本可接受；多实例部署下补偿任务须经分布式锁（Redis `SET NX` + TTL）保证单实例执行，避免重复调用 Embedding 浪费额度；
...
- 缺陷逻辑删除时同步逻辑删除 bug_embedding（同事务）。
...
3. **归纳**（40% → 90%）：按簇大小降序仅对前 `clustering.maxLabeledClusters`（settings 键，默认 30）个簇执行 LLM 归纳——取代表样本（离中心最近 5 条的标题+截断描述）归纳 `label` 与 `rootCause`（每簇一次调用，批间更新进度并检查取消标记），防止碎簇导致调用量失控；超出上限或 LLM 失败的簇保留 `label = "未命名主题 N"`；
...
2. 维度变更时：`DROP INDEX idx_*_hnsw` → `TRUNCATE bug_embedding / case_embedding`（向量数据无保留价值，源数据可再生）→ `ALTER TABLE … ALTER COLUMN embedding TYPE vector(N)` → 重建 HNSW 索引（空表建索引，后续增量写入自动维护）；
...
- 前端「AI 建议」按钮触发，按钮态 loading，结果以建议卡片展示于表单侧：优化标题（一键替换/忽略）、等级与优先级（点击采纳回填下拉框）；调用失败仅轻提示，不影响表单操作。
...
| 文件 | 说明 |
| ---- | ---- |
| `components/project/BugAiSuggest.vue` | 表单 AI 建议卡片（标题替换 / 等级采纳交互） |
| `components/project/BugDedupList.vue` | 疑似重复缺陷卡片列表（相似度徽标、状态、处理人；点击打开缺陷详情抽屉；降级模式提示条） |
| `components/project/BugClusterPanel.vue` | 缺陷看板「AI 分析」面板：聚类卡片（label/根因/缺陷数，点击展开缺陷清单并可跳转）+ 分布图 + 生成时间 + 刷新按钮 + 任务进行中进度条 |
| `composables/useBugDedup.ts` | 查重组合式函数：防抖、并发过期响应丢弃、6004 后停用自动触发（抽取以便单测，见 5.3） |
| `services/project.ts` / `types/index.ts` | 3.1–3.3 接口封装与类型 |

### 5.2 交互要点

- **查重触发**：缺陷新建/编辑表单监听标题（及描述）变更，防抖 500ms 调用 3.2；标题少于 5 字符不触发；查重请求进行中不阻塞输入与提交（并发以最后一次为准，过期响应丢弃）；收到 6004（频率超限）后本次表单会话内**停止自动查重**，仅保留「手动查重」按钮触发，避免自动触发持续撞限流；
...
- 查重防抖与过期响应丢弃逻辑（组合式函数抽取 `useBugDedup.ts` 便于单测）；
...
- **实施梯队**：表单建议属梯队二；语义查重属梯队三（向量基建随之上线）；聚类分析属梯队四；用例向量的消费功能（遗漏分析语义版、回归推荐）属梯队三，见计划辅助文档；
