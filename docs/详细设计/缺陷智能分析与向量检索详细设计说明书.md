...
> **V1.3 变更**：`clusters[]` 新增 `labeled` 字段（boolean）——LLM 归纳成功为 `true`；归纳失败（4.3 步骤 3 的异常兜底）或超出 `clustering.maxLabeledClusters` 未归纳、回退占位「未命名主题 N」时为 `false`。前端据此在主题卡片上明示「标签生成失败」，避免把占位标签误读为正常归纳结果（`labeled=false` 时 `rootCause` 恒为 `null`）。
