...
| 文件 | 说明 |
| ---- | ---- |
| `components/project/BugAiSuggest.vue` | 表单 AI 结果面板（建议区 + 查重区合一）：标题替换/等级采纳交互 + 内嵌 BugDedupList |
| `components/project/BugDedupList.vue` | 疑似重复缺陷卡片列表（纯展示：相似度徽标、状态、处理人；点击打开缺陷详情抽屉；降级模式提示条），数据由 BugAiSuggest 注入 |
