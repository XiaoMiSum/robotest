...
- **处理逻辑**：密码哈希后，在事务中插入 sys_user，若 roleIds 非空批量插入 sys_user_role，若 workspaceIds 非空批量插入 workspace_user（默认引用预置 `workspace_member` 角色 ID）。
...
POST /api/admin/users
  ...
  │     └── 若 workspaceIds 非空，批量插入 workspace_user 表（默认引用预置 workspace_member 角色 ID）
  ├── 5. 提交事务
