# 软件测试平台——系统管理模块详细设计说明书

**文档版本**：V1.0  
**日期**：2026-07-08  
**作者**：[填写]  
**状态**：已发布

---

## 1. 引言

### 1.1 编写目的

本文档对软件测试平台中的**系统管理模块**进行详细设计，定义数据结构、接口规范、业务逻辑、前端组件及交互流程，为开发实现提供完整依据。

### 1.2 范围

系统管理模块面向**拥有系统角色的用户**，提供用户管理、全局工作空间管理、系统角色与权限管理功能。模块独立于业务功能，通过 `/api/admin` 路径访问。

### 1.3 参考资料

- 《软件测试平台需求规格说明书 V2.1》
- 《软件测试平台概要设计说明书 V2.2》
- 《系统管理模块页面交互设计 V1.0》

---

## 2. 数据设计

### 2.1 数据库表设计

数据库字段使用 snake_case，接口 JSON 使用 camelCase。

#### 2.1.1 用户表（sys_user）

| 字段            | 类型                        | 约束                                    | 说明    |
| ------------- | ------------------------- | ------------------------------------- | ----- |
| id            | binary(16)                | PK                                     | 用户ID  |
| username      | varchar(30)               | UNIQUE, NOT NULL                      | 用户名   |
| email         | varchar(255)              | NOT NULL                              | 邮箱    |
| password_hash | varchar(255)              | NOT NULL                              | 密码哈希  |
| avatar_url    | varchar(500)              | NULL                                  | 头像URL |
| status        | enum('active','disabled') | NOT NULL, DEFAULT 'active'            | 状态    |
| created_at    | datetime                  | NOT NULL, DEFAULT CURRENT_TIMESTAMP   | 创建时间  |
| updated_at    | datetime                  | NOT NULL, ON UPDATE CURRENT_TIMESTAMP | 更新时间  |
| is_deleted    | tinyint(1)                | NOT NULL, DEFAULT 0                   | 是否删除  |

**索引**：`uk_username` UNIQUE (username), `idx_status` (status)

#### 2.1.2 角色表（sys_role）

| 字段          | 类型                        | 约束                                    | 说明        |
| ----------- | ------------------------- | ------------------------------------- | --------- |
| id          | binary(16)                | PK                                     | 角色ID      |
| name        | varchar(50)               | UNIQUE, NOT NULL                      | 角色名称      |
| description | varchar(200)              | NULL                                  | 描述        |
| type        | enum('system','business') | NOT NULL                              | 角色类型      |
| is_system   | tinyint(1)                | NOT NULL, DEFAULT 0                   | 是否预置角色    |
| permissions | json                      | NOT NULL                              | 权限点code列表 |
| created_at  | datetime                  | NOT NULL, DEFAULT CURRENT_TIMESTAMP   | 创建时间      |
| updated_at  | datetime                  | NOT NULL, ON UPDATE CURRENT_TIMESTAMP | 更新时间      |
| is_deleted  | tinyint(1)                | NOT NULL, DEFAULT 0                   | 是否删除      |

**索引**：`uk_name` UNIQUE (name), `idx_type` (type)

#### 2.1.3 用户-角色关联表（sys_user_role）

| 字段          | 类型       | 约束                                  | 说明   |
| ----------- | -------- | ----------------------------------- | ---- |
| id          | binary(16) | PK                                   | 关联ID |
| user_id     | binary(16) | NOT NULL                             | 用户ID |
| role_id     | binary(16) | NOT NULL                             | 角色ID |
| assigned_at | datetime   | NOT NULL, DEFAULT CURRENT_TIMESTAMP  | 分配时间 |
| created_at  | datetime   | NOT NULL, DEFAULT CURRENT_TIMESTAMP  | 创建时间 |
| updated_at  | datetime   | NOT NULL, ON UPDATE CURRENT_TIMESTAMP | 更新时间 |
| is_deleted  | tinyint(1) | NOT NULL, DEFAULT 0                  | 是否删除 |

**索引**：`uk_user_role` UNIQUE (user_id, role_id), `idx_role_id` (role_id)

#### 2.1.4 工作空间表（workspace）

| 字段          | 类型                         | 约束                                    | 说明     |
| ----------- | -------------------------- | ------------------------------------- | ------ |
| id          | binary(16)                 | PK                                     | 工作空间ID |
| name        | varchar(50)                | UNIQUE, NOT NULL                      | 名称     |
| description | varchar(500)               | NULL                                  | 描述     |
| status      | enum('active','dissolved') | NOT NULL, DEFAULT 'active'            | 状态     |
| created_at  | datetime                   | NOT NULL, DEFAULT CURRENT_TIMESTAMP   | 创建时间   |
| updated_at  | datetime                   | NOT NULL, ON UPDATE CURRENT_TIMESTAMP | 更新时间   |
| is_deleted  | tinyint(1)                 | NOT NULL, DEFAULT 0                   | 是否删除   |

**索引**：`uk_name` UNIQUE (name)

#### 2.1.5 用户-工作空间关联表（workspace_user）

| 字段             | 类型                     | 约束                                  | 说明      |
| -------------- | ---------------------- | ----------------------------------- | ------- |
| id             | binary(16)                 | PK                                   | 关联ID    |
| user_id        | binary(16)                 | NOT NULL                             | 用户ID    |
| workspace_id   | binary(16)                 | NOT NULL                             | 工作空间ID  |
| workspace_role | enum('member','admin')     | NOT NULL, DEFAULT 'member'           | 工作空间内角色 |
| joined_at      | datetime                   | NOT NULL, DEFAULT CURRENT_TIMESTAMP  | 加入时间    |
| is_deleted     | tinyint(1)                 | NOT NULL, DEFAULT 0                  | 是否删除    |

**索引**：`uk_user_workspace` UNIQUE (user_id, workspace_id), `idx_workspace_id` (workspace_id)

#### 2.1.6 权限点表（sys_permission）

| 字段          | 类型           | 约束                             | 说明     |
| ----------- | ------------ | ------------------------------ | ------ |
| id          | binary(16)  | PK                                  | 主键ID   |
| code        | varchar(100) | UNIQUE, NOT NULL               | 权限点编码  |
| name        | varchar(100) | NOT NULL                       | 权限点名称  |
| parent_code | varchar(100) | NULL                          | 父权限点编码 |
| module      | varchar(50)  | NOT NULL                       | 所属模块   |
| sort_order  | int          | NOT NULL, DEFAULT 0            | 排序号    |
| created_at  | datetime     | NOT NULL, DEFAULT CURRENT_TIMESTAMP  | 创建时间   |
| updated_at  | datetime     | NOT NULL, ON UPDATE CURRENT_TIMESTAMP | 更新时间   |
| is_deleted  | tinyint(1)   | NOT NULL, DEFAULT 0                  | 是否删除   |

**索引**：`uk_code` UNIQUE (code), `idx_parent_code` (parent_code), `idx_module` (module)

---

## 3. 接口详细设计

### 3.1 通用约定

- 基础路径：`/api/admin`
- 认证：`Authorization: Bearer <admin_token>`
- 命名风格：camelCase
- 分页：`page`、`pageSize` → `{ records: [], total: number }`
- 通用响应：`{ "code": 200, "message": "success", "data": {} }`

### 3.2 用户管理接口

#### 3.2.1 获取用户列表

- **路径**：`GET /api/admin/users`
- **请求参数**：

| 参数          | 类型     | 必填  | 说明                |
| ----------- | ------ | --- | ----------------- |
| keyword     | string | 否   | 按用户名/邮箱模糊搜索       |
| status      | string | 否   | active / disabled |
| roleId      | bigint | 否   | 筛选拥有该角色的用户        |
| workspaceId | bigint | 否   | 按所属工作空间筛选         |
| page        | int    | 否   | 页码，默认 1           |
| pageSize    | int    | 否   | 每页数量，默认 20，最大 100 |

- **响应数据**：
  
  ```json
  {
  "records": [
    {
      "id": 1,
      "username": "zhangsan",
      "email": "zhangsan@example.com",
      "avatarUrl": "https://...",
      "status": "active",
      "roles": [
        { "id": 2, "name": "用户管理员", "type": "system" }
      ],
      "workspaces": [
        { "id": 1, "name": "电商平台", "workspaceRole": "member" }
      ],
      "createdAt": "2026-01-15T10:30:00Z",
      "updatedAt": "2026-06-20T14:00:00Z"
    }
  ],
  "total": 45
  }
  ```

#### 3.2.2 创建用户

- **路径**：`POST /api/admin/users`

- **请求体**：
  
  ```json
  {
  "username": "lisi",
  "email": "lisi@example.com",
  "password": "Abc@123456",
  "roleIds": [2, 3],
  "workspaceIds": [1, 2]
  }
  ```

- **校验规则**：
  
  - `username`：必填，3~30 字符，正则 `^[a-zA-Z0-9_-]+$`，不可与已有用户名重复。
  - `email`：必填，合法邮箱格式，不可与已有邮箱重复。
  - `password`：必填，8~64 字符，须包含大写、小写、数字、特殊字符中至少三种。
  - `roleIds`：可选，允许为空数组。若不为空，所有ID必须存在于 sys_role 表且 `type='system'`。
  - `workspaceIds`：可选，可为空数组。

- **处理逻辑**：密码哈希后，在事务中插入 sys_user，若 roleIds 非空批量插入 sys_user_role，若 workspaceIds 非空批量插入 workspace_user（默认 workspace_role='member'）。

- **响应**：返回创建的用户完整信息，状态码 201。

#### 3.2.3 获取用户详情

- **路径**：`GET /api/admin/users/:id`
- **响应**：返回单个用户的完整信息，含 roles 列表和 workspaces 列表（字段同上）。

#### 3.2.4 更新用户

- **路径**：`PUT /api/admin/users/:id`

- **请求体**：
  
  ```json
  {
  "email": "newemail@example.com",
  "roleIds": [2],
  "workspaceIds": [1]
  }
  ```

- **校验规则**：
  
  - 不可修改 `username`。
  - `roleIds` 可选，允许为空数组；若不为空，所有角色必须为系统角色。
  - `workspaceIds` 可选，允许为空数组。
  - 不允许移除自己的最后一个系统角色。

- **处理逻辑**：更新 email，全量替换 sys_user_role（删除旧关联，插入新关联），全量替换 workspace_user（新增关联默认 member，保留已有关联的原角色，删除不再需要的关联）。若角色发生变更，强制该用户所有 Token 失效。

- **响应**：返回更新后的用户信息。

#### 3.2.5 更新用户状态

- **路径**：`PATCH /api/admin/users/:id/status`
- **请求体**：`{ "status": "disabled" }`
- **校验**：不可禁用自己。
- **处理**：更新 status 字段，若禁用则立即将用户所有 Token 加入黑名单或递增版本号。
- **响应**：返回更新后的用户信息。

#### 3.2.6 批量操作

- **路径**：`PATCH /api/admin/users/batch-status`
- **请求体**：`{ "userIds": [1, 2, 3], "status": "disabled" }`
- **校验**：批量操作中不可包含当前登录管理员自身。
- **响应**：返回成功和失败列表。

#### 3.2.7 重置密码

- **路径**：`POST /api/admin/users/:id/reset-password`
- **请求体**：`{ "newPassword": "NewPass@123" }`
- **校验**：新密码须符合密码强度规则。
- **处理**：更新密码哈希，强制该用户所有 Token 失效。
- **响应**：操作成功提示，不返回密码。

### 3.3 工作空间管理接口

#### 3.3.1 获取工作空间列表

- **路径**：`GET /api/admin/workspaces`
- **请求参数**：

| 参数       | 类型     | 必填  | 说明                |
| -------- | ------ | --- | ----------------- |
| keyword  | string | 否   | 按名称模糊搜索           |
| page     | int    | 否   | 页码，默认 1           |
| pageSize | int    | 否   | 每页数量，默认 20，最大 100 |

- **响应**：
  
  ```json
  {
  "records": [
    {
      "id": 1,
      "name": "电商平台测试",
      "description": "负责电商平台相关测试",
      "memberCount": 15,
      "projectCount": 5,
      "status": "active",
      "createdAt": "2026-01-01T00:00:00Z"
    }
  ],
  "total": 12
  }
  ```

#### 3.3.2 创建工作空间

- **路径**：`POST /api/admin/workspaces`
- **请求体**：`{ "name": "新项目工作空间", "description": "工作空间描述" }`
- **校验**：`name` 必填，2~50 字符，全局唯一。
- **响应**：返回创建的工作空间信息，状态码 201。

#### 3.3.3 获取工作空间详情

- **路径**：`GET /api/admin/workspaces/:id`
- **响应**：工作空间基本信息 + 成员列表（分页，成员信息含 workspaceRole）。

#### 3.3.4 更新工作空间

- **路径**：`PUT /api/admin/workspaces/:id`
- **请求体**：可更新 `name`、`description`。
- **校验**：名称唯一性，已解散的工作空间不可编辑。

#### 3.3.5 解散工作空间

- **路径**：`DELETE /api/admin/workspaces/:id`
- **前置校验**：工作空间下须无任何项目（含归档项目），否则返回可清理项目列表。
- **处理逻辑**：更新 status='dissolved'，删除 workspace_user 中所有关联记录。
- **响应**：成功提示。

#### 3.3.6 获取工作空间成员

- **路径**：`GET /api/admin/workspaces/:id/members`

- **请求参数**：`page`、`pageSize`

- **响应**：
  
  ```json
  {
  "records": [
    {
      "userId": 1,
      "username": "zhangsan",
      "email": "zhangsan@example.com",
      "workspaceRole": "admin",
      "joinedAt": "2026-01-15T10:30:00Z"
    }
  ],
  "total": 8
  }
  ```

#### 3.3.7 添加成员

- **路径**：`POST /api/admin/workspaces/:id/members`

- **请求体**：
  
  ```json
  {
  "members": [
    { "userId": 1, "workspaceRole": "member" },
    { "userId": 2, "workspaceRole": "admin" }
  ]
  }
  ```

- **校验**：用户ID必须存在且为活跃状态；已在本工作空间的用户跳过；`workspaceRole` 仅可为 `member` 或 `admin`，默认 `member`。

- **响应**：返回成功添加和跳过的用户列表。

#### 3.3.8 更新成员角色

- **路径**：`PUT /api/admin/workspaces/:id/members/:userId`
- **请求体**：`{ "workspaceRole": "admin" }`
- **校验**：降级唯一管理员时阻止。

#### 3.3.9 移除成员

- **路径**：`DELETE /api/admin/workspaces/:id/members/:userId`
- **校验**：不能移除空间最后一位管理员。
- **响应**：成功提示。

### 3.4 角色与权限管理接口

#### 3.4.1 获取角色树

- **路径**：`GET /api/admin/roles/tree`

- **说明**：按类型分组返回角色树，根节点为角色类型（system/business），子节点为具体角色。管理端仅展示系统角色。

- **响应**：
  
  ```json
  [
  {
    "id": "type-system",
    "name": "系统角色",
    "type": "system",
    "isGroup": true,
    "children": [
      {
        "id": 1,
        "name": "系统管理员",
        "type": "system",
        "isSystem": true,
        "userCount": 2
      },
      {
        "id": 2,
        "name": "用户管理员",
        "type": "system",
        "isSystem": false,
        "userCount": 5
      }
    ]
  }
  ]
  ```

#### 3.4.2 创建角色

- **路径**：`POST /api/admin/roles`
- **请求体**：`{ "name": "新角色", "type": "system" }`
- **校验**：`name` 唯一；`type` 仅允许 `"system"`；`permissions` 初始为空数组。
- **响应**：返回创建的角色信息，状态码 201。

#### 3.4.3 更新角色名称

- **路径**：`PUT /api/admin/roles/:id`
- **请求体**：`{ "name": "新名称" }`
- **校验**：名称唯一；仅允许修改系统角色。
- **响应**：返回更新后的角色信息。

#### 3.4.4 删除角色

- **路径**：`DELETE /api/admin/roles/:id`
- **校验**：系统预置角色（is_system=true）不可删除；若存在用户引用该角色，禁止删除并返回引用用户列表。
- **响应**：成功提示。

#### 3.4.5 获取角色详情

- **路径**：`GET /api/admin/roles/:id`
- **响应**：`{ id, name, description, type, isSystem, permissions: [], userCount }`

#### 3.4.6 获取角色关联用户

- **路径**：`GET /api/admin/roles/:id/users`
- **请求参数**：`page`、`pageSize`
- **响应**：`{ records: [{ id, username, email, status }], total }`

#### 3.4.7 添加角色关联用户

- **路径**：`POST /api/admin/roles/:id/users`
- **请求体**：`{ "userIds": [1, 2] }`
- **处理**：批量插入 sys_user_role，已存在的跳过。

#### 3.4.8 移除角色关联用户

- **路径**：`DELETE /api/admin/roles/:id/users/:userId`
- **处理**：删除对应的 sys_user_role 记录。

#### 3.4.9 更新角色权限

- **路径**：`PUT /api/admin/roles/:id/permissions`
- **请求体**：`{ "permissions": ["user:view", "user:create"] }`
- **校验**：系统预置角色的已有权限不可移除，只能追加新权限。
- **响应**：返回更新后的角色信息。

#### 3.4.10 获取权限点表格数据

- **路径**：`GET /api/admin/permissions/table`

- **说明**：返回按模块分组的权限点列表，供表格形式展示。

- **响应**：
  
  ```json
  [
  {
    "module": "用户管理",
    "permissions": [
      { "code": "user:view", "name": "查看用户" },
      { "code": "user:create", "name": "创建用户" },
      { "code": "user:edit", "name": "编辑用户" },
      { "code": "user:disable", "name": "禁用/启用用户" },
      { "code": "user:reset-password", "name": "重置密码" }
    ]
  },
  {
    "module": "工作空间管理",
    "permissions": [
      { "code": "workspace:view", "name": "查看工作空间" },
      { "code": "workspace:create", "name": "创建工作空间" },
      { "code": "workspace:edit", "name": "编辑工作空间" },
      { "code": "workspace:delete", "name": "解散工作空间" },
      { "code": "workspace:manage-members", "name": "管理成员" }
    ]
  },
  {
    "module": "角色管理",
    "permissions": [
      { "code": "role:view", "name": "查看角色" },
      { "code": "role:create", "name": "创建角色" },
      { "code": "role:edit", "name": "编辑角色" },
      { "code": "role:delete", "name": "删除角色" }
    ]
  }
  ]
  ```

---

## 4. 业务逻辑设计

### 4.1 用户创建流程

```
POST /api/admin/users
  ├── 1. 参数校验（用户名格式及唯一性、邮箱格式及唯一性、密码强度）
  ├── 2. 校验 roleIds 若不为空，角色必须存在且 type='system'
  ├── 3. 密码哈希处理（bcrypt / argon2）
  ├── 4. 开启事务
  │     ├── 插入 sys_user 表
  │     ├── 若 roleIds 非空，批量插入 sys_user_role 表
  │     └── 若 workspaceIds 非空，批量插入 workspace_user 表（默认 workspace_role='member'）
  ├── 5. 提交事务
  └── 6. 返回用户完整信息（含 roles 和 workspaces）
```

### 4.2 用户更新流程

```
PUT /api/admin/users/:id
  ├── 1. 校验目标用户存在，且不允许修改 username
  ├── 2. 校验 roleIds（若不为空）有效且为系统角色
  ├── 3. 不能移除自身的最后一个系统角色
  ├── 4. 开启事务
  │     ├── 更新 sys_user.email
  │     ├── 全量替换 sys_user_role（删除旧关联，插入新关联）
  │     └── 合并更新 workspace_user：新增关联默认 member，保留已有关联的原角色，删除不再需要的关联
  ├── 5. 若角色发生变更，强制该用户所有 Token 失效
  └── 6. 返回更新后信息
```

### 4.3 角色管理流程

#### 4.3.1 创建角色

- 点击类型根节点（如“系统角色”）的[+]按钮，前端在树中新增可编辑节点。
- 输入角色名称，失焦后调用 `POST /api/admin/roles`，提交 `name` 和 `type='system'`。
- 后端校验名称唯一性，permissions 初始化为空数组，插入 sys_role 表。
- 返回新角色信息，前端更新树节点。

#### 4.3.2 编辑角色名称

- 双击树节点进入编辑模式，修改名称后失焦自动调用 `PUT /api/admin/roles/:id`。
- 后端校验名称唯一性，更新 sys_role.name。
- 成功后更新树节点显示。

#### 4.3.3 删除角色

- 右键角色节点或选中后点击删除按钮，前端弹出二次确认弹窗。
- 确认后调用 `DELETE /api/admin/roles/:id`。
- 后端校验：系统预置角色（is_system=true）不可删除；若 sys_user_role 中存在引用，返回引用用户列表，阻止删除。
- 成功后从树中移除节点。

#### 4.3.4 权限配置

- 单击角色节点，右侧加载角色详情，默认激活“权限配置”Tab。
- 调用 `GET /api/admin/roles/:id` 获取角色已有权限列表，调用 `GET /api/admin/permissions/table` 获取全部权限点。
- 前端渲染权限表格，每行一个模块，权限点列为复选框。已在角色权限列表中的 code 勾选。
- 用户可勾选/取消复选框。系统预置角色的既有权限复选框禁用（灰显不可取消）。
- 点击[撤销修改]：恢复到上次保存时的权限快照。
- 点击[保存权限]：调用 `PUT /api/admin/roles/:id/permissions`，提交当前选中的权限 code 列表。
- 后端校验系统预置角色权限不可移除，保存并返回更新后信息。

#### 4.3.5 关联用户管理

- 切换到“关联用户”Tab，调用 `GET /api/admin/roles/:id/users` 分页加载关联用户列表。
- 点击[添加用户]弹出搜索弹窗，支持多选和远程搜索活跃用户，提交后调用 `POST /api/admin/roles/:id/users` 批量插入 sys_user_role。
- 每行用户有[移除]按钮，点击二次确认后调用 `DELETE /api/admin/roles/:id/users/:userId`。
- 操作完成后刷新列表。

### 4.4 权限校验中间件

- 后端实现授权中间件，在管理端路由组统一挂载。
- 从 JWT 提取 userId 和所有角色，查询合并后的权限 code 列表。
- 判断当前请求所需的权限 code 是否在列表中。所需权限 code 由路径和方法映射确定。

权限 code 与路径映射：

| 路径                                  | 方法     | 所需权限 code                |
| ----------------------------------- | ------ | ------------------------ |
| /api/admin/users                    | GET    | user:view                |
| /api/admin/users                    | POST   | user:create              |
| /api/admin/users/:id                | PUT    | user:edit                |
| /api/admin/users/:id/status         | PATCH  | user:disable             |
| /api/admin/users/:id/reset-password | POST   | user:reset-password      |
| /api/admin/workspaces               | GET    | workspace:view           |
| /api/admin/workspaces               | POST   | workspace:create         |
| /api/admin/workspaces/:id           | DELETE | workspace:delete         |
| /api/admin/workspaces/:id/members   | POST   | workspace:manage-members |
| /api/admin/roles/tree               | GET    | role:view                |
| /api/admin/roles                    | POST   | role:create              |
| /api/admin/roles/:id                | PUT    | role:edit                |
| /api/admin/roles/:id                | DELETE | role:delete              |

### 4.5 用户禁用与强制下线

- 管理员将用户状态置为 disabled 后，系统立即将该用户所有活跃 Token 加入 Redis 黑名单或递增 token 版本号。
- 网关中间件验证 Token 时，检查用户状态及 token 版本，不匹配返回 401（错误码 2005）。
- 密码重置后同样触发 Token 失效。

### 4.6 密码策略

- 强度：8~64 字符，包含大写字母、小写字母、数字、特殊字符中至少三种。
- 存储：使用 bcrypt（cost ≥ 10）或 argon2id 单向哈希。
- 重置密码：管理员输入新密码，后端不返回明文。

---

## 5. 前端设计

### 5.1 路由规划

```
/admin
├── /dashboard            → DashboardPage（仪表盘）
├── /users                → UserListPage（用户列表）
├── /users/create         → UserCreatePage（新建用户）
├── /users/:id            → UserDetailPage（用户详情/编辑）
├── /workspaces           → WorkspaceListPage（工作空间列表）
├── /workspaces/create    → WorkspaceCreatePage（新建工作空间）
├── /workspaces/:id       → WorkspaceDetailPage（工作空间详情/成员管理）
└── /roles                → RoleManagementPage（角色管理，左右分栏）
```

### 5.2 页面组件树

#### 5.2.1 AdminLayout

```
AdminLayout
├── Sidebar
│   ├── Logo
│   ├── NavItem (仪表盘)
│   ├── NavItem (用户管理)
│   ├── NavItem (工作空间管理)
│   └── NavItem (角色管理)
├── Header
│   ├── Breadcrumb
│   └── UserMenu (当前管理员信息/退出)
└── Content (RouterView)
```

#### 5.2.2 角色管理页（RoleManagementPage）

```
RoleManagementPage
├── RoleTreePanel（左侧，宽度260px）
│   ├── el-tree
│   │   ├── 根节点（系统角色） [+ 按钮]
│   │   │   ├── 系统管理员
│   │   │   ├── 用户管理员
│   │   │   └── ...
│   │   └── 根节点（业务角色）仅展示，无[+]
│   └── 右键菜单（删除角色）
└── RoleDetailPanel（右侧）
    ├── el-tabs
    │   ├── Tab1: 权限配置 (PermissionTable)
    │   └── Tab2: 关联用户 (RoleUsersTable)
    └── 操作按钮 ([撤销修改] [保存权限])
```

#### 5.2.3 PermissionTable组件

使用 el-table 渲染权限点，列为“操作对象”和“权限点”。权限点列中每个权限渲染为 el-checkbox。系统预置角色的已有权限复选框添加 disabled 属性。

#### 5.2.4 RoleUsersTable组件

表格展示关联用户（用户名、邮箱、状态、操作）。顶部[添加用户]按钮，每行[移除]按钮。

### 5.3 状态管理（Pinia Store）

- `adminUser`：当前管理员信息及系统角色列表
- `systemRoles`：系统角色列表（用于选择器）
- `workspaceList`：工作空间简要列表（用于选择器）
- `permissionTableData`：权限表格数据缓存
- `roleTreeData`：角色树数据

### 5.4 关键组件交互

#### 5.4.1 角色树组件（RoleTreePanel）

- **数据源**：`GET /api/admin/roles/tree` 返回的树形数组，前端使用 el-tree 渲染。
- **添加角色**：点击类型根节点的[+]按钮，在对应类型下插入新节点（默认名称空，输入框自动聚焦）。输入名称后失焦，校验通过则调用创建角色接口，成功后更新树。
- **编辑名称**：双击节点进入编辑模式，修改后失焦调用更新角色接口。
- **删除角色**：右键菜单或选中节点后工具栏删除按钮，二次确认后调用删除接口。
- **选择角色**：单击节点，右侧加载角色详情，默认激活“权限配置”Tab。

#### 5.4.2 权限表格组件（PermissionTable）

- **数据源**：`GET /api/admin/permissions/table` 返回的模块分组数组。
- **勾选状态**：根据角色已有权限 code 列表初始化复选框状态。用户勾选/取消时更新本地快照。
- **撤销修改**：点击[撤销修改]按钮，恢复到上次保存时的权限快照。
- **保存权限**：点击[保存权限]按钮，收集所有选中 code，调用更新角色权限接口。成功后更新本地快照，Toast提示。

#### 5.4.3 关联用户组件（RoleUsersTable）

- **数据源**：`GET /api/admin/roles/:id/users` 分页加载。
- **添加用户**：点击[添加用户]弹出搜索弹窗，支持多选和远程搜索活跃用户，提交后调用批量添加接口。
- **移除用户**：每行[移除]按钮，二次确认后调用移除接口，刷新列表。

#### 5.4.4 角色选择器（用户表单中使用）

- 多选下拉，仅显示系统角色（调用 `GET /api/admin/roles` 过滤 type='system'）。
- 允许不选任何角色。
- 创建/编辑用户时使用，支持搜索过滤。

#### 5.4.5 工作空间选择器（用户表单中使用）

- 多选下拉，远程搜索（防抖300ms），展示名称和成员数。
- 在用户编辑界面中仅设置归属，无法设置工作空间内角色。

---

## 6. 错误码定义

| 错误码  | HTTP 状态码 | 说明               |
| ---- | -------- | ---------------- |
| 1001 | 400      | 参数校验失败           |
| 1002 | 400      | 用户名已存在           |
| 1003 | 400      | 邮箱已存在            |
| 1004 | 400      | 工作空间名称已存在        |
| 1005 | 400      | 角色名称已存在          |
| 1006 | 400      | 密码强度不符合要求        |
| 1008 | 400      | 角色类型错误（只能选择系统角色） |
| 1009 | 400      | 必须保留至少一个空间管理员    |
| 2001 | 403      | 无权限执行此操作         |
| 2002 | 403      | 不可操作自身账户         |
| 2003 | 403      | 系统预置角色不可删除       |
| 2004 | 403      | 系统预置角色权限不可修改     |
| 2005 | 401      | 账户已被禁用或登录凭证失效    |
| 2006 | 403      | 不能移除自己的最后一个系统角色  |
| 3001 | 404      | 用户不存在            |
| 3002 | 404      | 工作空间不存在          |
| 3003 | 404      | 角色不存在            |
| 4001 | 409      | 工作空间下存在项目，无法解散   |
| 4002 | 409      | 角色被用户引用，无法删除     |
| 5000 | 500      | 服务器内部错误          |

---

## 7. 安全设计

- 管理端所有接口要求 Token 中包含系统角色。
- 任何角色变更、密码重置、禁用操作都强制相关用户 Token 失效（Redis 黑名单或 token 版本号递增）。
- 所有写操作（创建、更新、删除）记录操作日志（操作人、时间、IP、操作对象、变更内容）。
- 敏感操作（重置密码、解散工作空间、删除角色）额外记录详细日志。

---

## 8. 附录

### 8.1 初始权限点定义

| code                     | 名称      | parent_code | module | 排序  |
| ------------------------ | ------- | ----------- | ------ | --- |
| user                     | 用户管理    | null        | admin  | 1   |
| user:view                | 查看用户    | user        | admin  | 1   |
| user:create              | 创建用户    | user        | admin  | 2   |
| user:edit                | 编辑用户    | user        | admin  | 3   |
| user:disable             | 禁用/启用用户 | user        | admin  | 4   |
| user:reset-password      | 重置密码    | user        | admin  | 5   |
| workspace                | 工作空间管理  | null        | admin  | 2   |
| workspace:view           | 查看工作空间  | workspace   | admin  | 1   |
| workspace:create         | 创建工作空间  | workspace   | admin  | 2   |
| workspace:edit           | 编辑工作空间  | workspace   | admin  | 3   |
| workspace:delete         | 解散工作空间  | workspace   | admin  | 4   |
| workspace:manage-members | 管理成员    | workspace   | admin  | 5   |
| role                     | 角色管理    | null        | admin  | 3   |
| role:view                | 查看角色    | role        | admin  | 1   |
| role:create              | 创建角色    | role        | admin  | 2   |
| role:edit                | 编辑角色    | role        | admin  | 3   |
| role:delete              | 删除角色    | role        | admin  | 4   |

### 8.2 预置角色

- **系统管理员**（type='system', is_system=true）：拥有 admin 模块所有权限。
- **业务用户**（type='business', is_system=true）：拥有 workspace 模块基本权限，不在管理端管理。

---

**文档结束**
