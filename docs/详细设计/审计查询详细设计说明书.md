...
- **审计写入（登录路径，V1.2 增量）**：登录成功后写入 `operation='LOGIN'` 记录（含登录 IP），供数据概览活跃统计与后续审计页消费（见 4.3）。
- **审计查询**：分页查询 `GET /api/admin/audit-logs`（V1.2 新增 `operation` 过滤，用于按登录/业务操作区分）、聚合统计 `GET /api/admin/audit-logs/aggregate` 两个端点。
...
**operation 取值（V1.2）**：`CREATE` / `UPDATE` / `DELETE`（注解路径）+ `LOGIN`（登录路径增量）。`LOGIN` 记录结构：`entity_type='User'`、`entity_id=operator_id=用户 ID`、`operator_name=登录用户名`、`request_ip=登录 IP`、`changes={}`；`created_at` 落在今日/近 14 日的 `LOGIN` 记录即数据概览「今日活跃 / 近 14 日活跃用户」的统计源（口径见 `docs/详细设计/系统管理模块详细设计说明书.md` §3.2）。
...
| operation | String | 否 | 操作类型精确过滤（如 `LOGIN` / `CREATE` / `UPDATE` / `DELETE`，V1.2 新增） |
...
- `AuditLogAspect` 追加 `ApplicationEventPublisher.publishEvent(...)`（写入不变量，失败不影响主事务）；V1.2 将写入 + 事件发布下沉至共享 `AuditLogWriter`，切面行为不变。
- V1.2 增量：`framework/audit/ClientIpResolver`、`LoginAuditService`（recordLogin 组装与容错）、`AuditLogController/AuditQueryService/AuditLogMapper` 增加 `operation` 过滤参数。
