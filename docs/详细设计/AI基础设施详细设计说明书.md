...
默认模板初始化时**全量落库**（`db/v1.1.sql` 种子数据，与代码内置资源同源），管理端可查看并修改；恢复默认即逻辑删除该行并回退代码内置默认。
...
列表为全部有模板位的功能类型（见 2.3），`customized` 表示存在数据库记录（初始化种子落库后均为 true；恢复默认逻辑删除后为 false）。
...
```json
{
  ...
  "defaults": {
    "roleInstruction": "内置默认角色指令段…",
    "formatConstraint": "内置默认格式约束段…"
  }
}
```
...
- **处理**：无记录则插入（按 2.1.2 种子语义仅恢复默认后可能出现），有则更新；变更写 sys_audit_log。

#### 3.4.4 恢复默认

- **路径**：`DELETE /api/admin/ai/agents/:functionType`
- **处理**：逻辑删除该记录，回退代码内置默认（与种子数据同源）；写 sys_audit_log。无记录时幂等成功。
