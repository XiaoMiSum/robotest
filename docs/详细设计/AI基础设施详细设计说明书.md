...
仅存**自定义覆盖**记录；默认模板内置于代码，恢复默认即逻辑删除该行。
...
列表为代码内置的全部模板位（见 2.3 有模板位的功能类型），`customized` 表示存在自定义覆盖记录。
...
- **模板加载**：按 `function_type` 查 `ai_prompt_template` 有效记录，命中用自定义段，未命中用代码内置默认（内置模板以资源文件形式随代码维护，键与 2.3 枚举一致）；
- **消息结构**：
