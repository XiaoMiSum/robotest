...
> 号段为文档简写。接口实际返回平台统一十位全码：**7701 ≙ 1000017701、7702 ≙ 1000017702**，映射在各端 `ErrorCodeConstants.java` 号段注释中登记；前端与联调以实际响应 `code` 为准。
...
遵循《API 测试基础设施详细设计说明书》3.1：项目级路径 `/api/project/**`，请求头 `Authorization` + `X-Active-Workspace` + `X-Active-Project`（C4）；服务端以 `X-Active-Workspace` 作为权限判定上下文，并校验 `X-Active-Project` 对应项目归属于该工作空间，不一致按项目不存在处理；响应 `{ "code": 200, "message": "success", "data": {} }`，下文示例仅展示 `data` 字段内容。
...
- **说明**：批量更新设置项，逐键 upsert。仅项目维护者可调用；权限不足时按平台统一错误约定返回业务码 `NO_PERMISSION`（HTTP 状态仍为 200，格式见 3.1 与《API 测试基础设施详细设计说明书》3.1），不使用独立 HTTP 403。
- **请求体**：
