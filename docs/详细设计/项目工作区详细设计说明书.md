...
| status              | enum('active','resolved','closed')        | NOT NULL, DEFAULT 'active'             | 状态（激活/已解决/已关闭）       |
...
- **解决** active → resolved：必填 resolution；resolution=duplicate 时必填 duplicate_of_bug_id（校验存在、非自身且同项目）；记录 resolved_by/resolved_at。
- **关闭** resolved → closed：必填说明；记录 closed_by/closed_at。
- **重开（激活）** resolved/closed → active：必填说明；reopen_count+1、last_reopened_at=now，清空 resolution/duplicate_of_bug_id/resolved_by/resolved_at/closed_by/closed_at。
...
- **参数**：`status`、`severity`、`priority`、`bugType`、`assigneeId`、`keyword`，分页。
...
- **请求体**：可更新 title、severity、priority、bugType、reproSteps、moduleId、keywords、dueDate、assigneeId（status 不可通过本接口修改，须走 3.6.5 状态流转接口）。
- **处理**：缺陷已关闭（closed）时拒绝编辑（错误码 1000012019），须先激活；修改 assigneeId 时校验其为当前工作空间成员；记录变更到 bug_log。
...
- **请求体**：`{ "status": "resolved", "resolution": "fixed", "duplicateOfBugId": "<uuid>", "comment": "变更说明" }`（resolution/duplicateOfBugId 仅 status=resolved 时使用）
- **处理**：按三态状态机校验流转合法性（active→resolved、resolved→closed/active、closed→active）；解决时 resolution 必填，resolution=duplicate 时 duplicateOfBugId 必填（校验存在、非自身且同项目），并自动置 confirmed=true（解决即视为确认）；关闭与重开时 comment 必填；重开时 reopen_count+1 并清空解决/关闭字段；写入 bug_log（RESOLVE/CLOSE/REOPEN）。
...
| 更新状态 | 在看板中拖拽卡片至目标列 | 拖至「已解决」弹解决对话框（选择 resolution），拖至关闭/激活弹说明输入；非法目标列置灰 |
| 确认/解决/关闭/激活 | 详情页状态栏按钮     | 按三态状态机流转，解决弹 BugResolveDialog |
| 列表行操作 | 操作列按钮：详情始终显示；按状态直出解决（active）/关闭（resolved）/激活（closed）；非 closed 行收「更多」下拉（确认、激活、指派） | 解决弹 BugResolveDialog，关闭/激活弹说明输入，指派弹成员选择对话框 |
| 查看详情 | 点击卡片         | 进入详情页（重现步骤 Markdown 渲染） |
| 筛选   | 选择状态/类型/严重等级/优先级 | 列表/看板刷新     |
