...
- **请求体**：可更新 title、severity、priority、description、assigneeId（status 不可通过本接口修改，须走 3.6.5 状态流转接口）。
...
- **请求体**：`{ "status": "fixing", "comment": "变更说明" }`
- **处理**：按状态机校验流转合法性（new→assigned→fixing→fixed→verified→closed，verified/closed 可重开回 fixing）；关闭与重开时 comment 必填；写入 bug_log。
