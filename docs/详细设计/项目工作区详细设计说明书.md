...
- **参数**：`status`（可选，in_progress/completed）、`keyword`（可选，标题模糊匹配）、`page`、`pageSize`
...
- **状态校验**：评审非 in_progress 时拒绝，返回语义化错误码 1000011026（“评审不在进行中状态，无法执行该操作”）。
...
- **权限**：仅发起人。
- **前端校验**：待评审数与不通过数均为 0（全部通过）才允许提交，否则按钮置灰并提示。
...
- **前置**：评审状态为 in_progress（未结束即可调整，不限发起人）。
...
- **校验**：仅 is_associated=true 且 type='case' 的节点可标记执行结果；result 取值限 `pass` / `fail` / `block` / `untested`（untested 即重置回待执行）；计划非 in_progress 时拒绝。执行结果以徽标形式回显在节点右侧。
