...
- **处理**：发现人默认为当前用户，状态初始为 active；bugType 必填且限枚举值；moduleId 非空时校验属于当前项目的 test_case_module；reproSteps 以 Markdown 原文存储；写入 bug_log。
...
- **处理**：修改 assigneeId 时校验其为当前工作空间成员；记录变更到 bug_log。
...
- **处理**：按三态状态机校验流转合法性（active→resolved、resolved→closed/active、closed→active）；解决时 resolution 必填，resolution=duplicate 时 duplicateOfBugId 必填（校验存在、非自身且同项目）；关闭与重开时 comment 必填；重开时 reopen_count+1 并清空解决/关闭字段；写入 bug_log（RESOLVE/CLOSE/REOPEN）。
