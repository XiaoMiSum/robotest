...
- 预览-确认阶段：生成弹窗保持打开（居中模态）；done 后点击 [查看预览] 打开独立预览弹窗（两窗并存、预览置顶）；预览弹窗内为 kityminder 只读实例渲染的本地快照（`buildPreviewTree`，仅生成节点树，文档既有数据不并入预览），不落库、不产生撤销历史；确认挂载成功 → 关闭全部弹窗；未确认挂载（仅关闭预览弹窗）→ 生成弹窗保留「完成」态、快照可重新预览；
