...
sequenceDiagram
    ...
    FE->>FE: 组装完整文档树预览(前端活树快照+生成节点插入目标位)
    FE->>FE: 打开独立预览弹窗(脑图只读实例,AI节点带勾选框默认全选)
    U->>FE: 预览弹窗点击AI节点勾选取舍(级联)
    U->>FE: 预览弹窗确认挂载
    FE->>FE: 校验目标节点仍存在
    FE->>FE: 挂载执行器: 编辑内核批量addNode(单撤销组)
    FE->>WS: 既有 add_node 帧(含 aiGenerated) × N
    WS-->>WS: diff 持久化落库 + 协同广播
    FE->>FE: 挂载成功 → 关闭预览弹窗与生成弹窗(全部弹窗)
```

- 预览为**纯前端本地快照**（读取当前脑图活树 + 生成结果组装），不写编辑内核、不产生撤销历史、不广播、不落库；预览弹窗内勾选状态（`aiSelected`）仅存于该弹窗的只读 Minder 实例内存，关闭预览弹窗即丢弃；挂载确认后才经编辑内核写入（纯预览约束，见交互设计 2.2）；
- 挂载前目标节点已被协同用户删除 → 提示重选挂载位置（弹出节点选择器），不丢弃预览结果；
- 生成弹窗在生成过程中随 `delta` 帧显示原始流式文本，`done` 后底部变为 [重新生成] [查看预览]；点击 [查看预览] 打开独立预览弹窗（两窗并存、预览置顶）；关闭预览弹窗仅关预览、生成弹窗保留「完成」态。
...
- 预览阶段（`buildDocumentPreviewTree`）读取脑图活树快照组装完整文档树：既有节点只读展示，生成节点以 AI 徽标插入目标节点下、带勾选框默认全选（`aiSelected: true`）；**预览纯本地组装，不调用编辑内核**，勾选过滤（`filterCheckedTree`）仅提取 `aiGenerated && aiSelected` 的节点，仅确认挂载后才执行本节的批量写入；
- 勾选取舍规则：仅生成节点可勾选；取消父节点则其 AI 子孙一并排除；case 节点被取消时其 precondition/step/expected 子节点整组排除；恢复勾选父节点时其 AI 子孙整组恢复；
...
| `components/project/minder/ai/AiGeneratePanel.vue` | 「AI 生成用例」居中弹窗：需求文本域 / 条目选择 / 流式输出区；done 后提供 [重新生成] [查看预览]，不再内嵌预览树 |
| `components/project/minder/ai/AiPreviewDialog.vue` | 独立预览弹窗（约 920px）：弹窗内创建 kityminder 只读实例渲染完整文档树快照，AI 节点带勾选框（点击切换，父子级联），底部 [确认挂载] [关闭]；两窗并存、预览置顶 |
...
| `components/project/minder/ai/aiMount.ts` | 挂载执行器（4.2）：`AiPreviewNode` 携带 `aiSelected`；`buildDocumentPreviewTree` / `filterCheckedTree` 按勾选状态过滤 |
...
- 预览-确认阶段：生成弹窗保持打开（居中模态）；done 后点击 [查看预览] 打开独立预览弹窗（两窗并存、预览置顶）；预览弹窗内为 kityminder 只读实例渲染的本地快照（`buildDocumentPreviewTree`），不落库、不产生撤销历史；确认挂载成功 → 关闭全部弹窗；未确认挂载（仅关闭预览弹窗）→ 生成弹窗保留「完成」态、快照可重新预览；
...
- `aiMount.ts`：勾选过滤规则（`aiSelected` 父子联动、既有节点穿透不输出）、aiGenerated 写入、目标节点缺失分支（生成重选与补全不可挂载两种场景）；
