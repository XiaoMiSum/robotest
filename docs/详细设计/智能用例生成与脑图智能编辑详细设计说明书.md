...
| `components/project/minder/ai/AiGeneratePanel.vue` | 「AI 生成用例」抽屉（右侧滑出约 720px、透明遮罩不压暗画布，常驻挂载、关闭仅隐藏）：需求文本域 / 条目选择 / 流式输出区；done 后提供 [重新生成] [查看预览]，不再内嵌预览树；watch `docId` 切换文档时断开 SSE 并重置 |
