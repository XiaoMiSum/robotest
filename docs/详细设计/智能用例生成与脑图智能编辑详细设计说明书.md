...
本文档对 V1.1 AI 能力域中**围绕脑图编辑器的 AI 功能**进行详细设计：用例子树生成、步骤补全、优先级推荐、轻量需求池、批量指令操作（DSL）与外部文本导入，为开发实现提供完整依据。

### 1.2 范围

覆盖 SRS 3.2（智能测试用例生成）与 3.6（脑图智能编辑），含配套的需求池实体模块。公共基础（AI 网关、SSE 帧格式、限流审计、错误码 6001–6012）见《AI 基础设施详细设计说明书》，本文档不重复。

核心机制约束（概要 AD-3 / 4.2）：**AI 产出进入脑图一律经前端编辑内核挂载**，复用协同广播、diff 持久化与撤销链路，后端不提供批量写节点接口。

### 1.3 参考资料

- 《软件测试平台需求规格说明书 V1.1》（3.2、3.6）
...
#### 2.1.2 文档需求关联表（document_requirement_rel）
...
**索引**：`uk_doc_req` UNIQUE (document_id, requirement_id) WHERE is_deleted = false，`idx_drr_requirement_id` (requirement_id)
...
不新增索引（该字段仅用于渲染与 DSL 筛选，不作为独立查询条件）。快照创建逻辑（评审/计划的既有拷贝 SQL）同步增加该列的复制。

### 2.2 结构化输出数据结构（AI 生成用例树）

用例子树生成、步骤补全、外部文本导入共用同一输出结构（网关侧按下述规则校验，见基础设施文档 4.4）：
...
- **删除处理**：逻辑删除条目，同事务解除其全部文档关联（document_requirement_rel 逻辑删除）；不影响已生成的用例。
...
- `modelId`：可选，用户选择的对话模型标识（交互式功能通用约定，缺省或失效回退系统默认模型，见基础设施 3.1 / 4.11）；本节 3.2.2 / 3.2.3 与 3.3.2 的请求体同样支持该字段，不再逐一标注。
- **校验**：`requirementText` 与 `requirementIds` 至少一项非空；`targetNodeId` 必填——用户未选中节点时由前端传入文档根节点 ID（SRS 3.2.1：默认挂载根节点）；文档编辑权限；`saveAsRequirement` 非空时 `requirementText` 必须非空，生成开始**前**独立保存为需求池条目（与生成非同一事务），保存失败不阻断生成、在 `done` 帧 `warnings` 中提示。
- **流式响应**：`delta` 帧透传生成文本；`done` 帧为 2.2 结构 + `"warnings": []`（截断等提示）。
- **说明**：`targetNodeId` 仅用于生成上下文（祖先路径标题链 + 目标节点直接子节点标题——作为新子树的同级参照，见 4.7），挂载由前端执行；生成期间文档可继续编辑。
...
- **路径**：`POST /api/project/ai/minder/import`（SSE）
- **请求体**：`{ "documentId", "targetNodeId", "text": "外部需求/表格文本" }`
- **校验**：`text` 长度 ≤ `importTextMaxLength`（默认 20000），超限返回 1001；`targetNodeId` 必填，未选中节点时默认根节点（同 3.2.1）；支持自由文本 / Markdown / 制表符分隔内容，格式识别交由 LLM（提示词内声明三种格式的解析规则）。
- **done 帧**：2.2 完整树结构。
...
- 预览面板在生成过程中随 `delta` 帧显示原始流式文本，`done` 后切换为结构化树。

### 4.2 挂载执行器（前端 `aiMount.ts`）

「生成子树」「补全步骤」「文本导入」「对话式编辑新增节点」共用同一执行器：
...
- 每个新节点触发既有 `add_node` WS 帧，帧内节点数据携带 `aiGenerated` 字段（协议扩展见 4.6）；
- 勾选取舍规则：取消父节点则其子孙一并排除；case 节点被取消时其 precondition/step/expected 子节点整组排除；
- 目标节点缺失处理按场景区分：生成子树 / 文本导入 → 提示重选挂载位置（4.1，弹出节点选择器）；补全步骤的挂载目标只能是原 case 节点，该节点已被协同删除时提示"节点已被删除，无法挂载"，不提供重选。
...
- 入口与「AI 指令」共用输入区（见 5.2），文本长度超过阈值（200 字符）或含制表符/多行时自动按导入解析处理，用户也可显式切换模式；自动判定结果以模式徽标显式展示在输入区，判定有误时一键切回指令模式，已输入文本保留；
- 解析链路与生成子树一致（SSE → 预览 → 勾选 → 挂载）；提示词约束：表格文本按「一行一用例」解析，自由文本按语义分组；无法解析出任何用例结构时 `done` 帧返回空 `nodes` + warning，前端提示调整文本。

### 4.6 AI 标识（aiGenerated）全链路
...
| `components/project/RequirementSelector.vue` | 条目选取器弹窗（多选 + 关键字过滤 + 「保存为需求池条目」入口），供各 AI 入口复用 |
| `components/project/minder/ai/AiGeneratePanel.vue` | 「AI 生成用例」抽屉：需求文本域 / 条目选择 / 流式输出区 / 预览树 |
| `components/project/minder/ai/AiCommandBar.vue` | 「AI 指令」输入区（指令翻译与文本导入双模式） |
| `components/project/minder/ai/AiPreviewTree.vue` | 预览树（el-tree 级联勾选，展示类型/优先级标记与截断警告） |
| `components/project/minder/ai/aiMount.ts` | 挂载执行器（4.2） |
...
- 脑图工具栏新增「AI 生成用例」「AI 指令」按钮（`stores/ai.ts` 的 `aiEnabled` 控制显隐）；右键菜单在 case 节点上显示「AI 补全步骤」；
- 生成/补全/导入/指令翻译为交互式功能：`AiGeneratePanel` 与 `AiCommandBar` 内嵌公共组件 `AiModelSelect`（对话模型选择器，基础设施 5.1 / 交互设计 2.8），所选 `modelId` 随 3.2 / 3.3.2 请求提交；
- 全部 SSE 消费走基础设施的 `useAiStream()`（支持取消按钮、超 10 秒未见首帧提示可取消重试）；
- 预览-确认阶段面板保持打开，用户可切换到脑图继续编辑；挂载确认时执行目标节点存在性校验；
...
- `aiMount.ts`：勾选过滤规则（父子联动）、aiGenerated 写入、目标节点缺失分支（生成/导入重选与补全不可挂载两种场景）；
...
- **实施梯队**：子树生成 / 补全 / 文本导入属梯队一（此时需求条目选取降级为仅手动输入）；需求池属梯队二；优先级推荐、DSL 批量指令属梯队四；
- **依赖**：无新增前后端依赖。
