...
| A3 | ~~C4 违例：workspaceId 入 URL~~ → **撤回**：admin 跨空间角色管理（移除"某工作区×角色×成员"关系），workspaceId 为业务参数而非"用户当前激活上下文"，不属 C4 | `AdminRoleController.java:91-95` `@DeleteMapping("/{id}/users/{userId}/workspace/{workspaceId}")`（与 `addWorkspaceRoleUsers` 的 body 批量 workspaceIds 同域） | — | **误判，撤回** |
...
| C4 局部违例（URL/body） | 中 | `WorkspaceController`（用户端上下文操作） | `02`/`07` 增补 |
