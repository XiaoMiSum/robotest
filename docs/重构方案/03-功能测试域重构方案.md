...
**迁移矩阵**（源自 `TestReviewServiceImpl` :147 创建=NEW 直写 / :246 首次记录经 `ReviewWorkflow.transition`→IN_PROGRESS（仅 mark 触发）/ :308 完成=COMPLETED 直写 / 禁写守卫 `assertTransition` :207·:222·:305·:386；其中**创建与终态为纯写值直落**（非跃迁裁决，不迁状态机），仅跃迁类经 `transition`；缺失证据的格子以 P0 特征测试冻结为最终口径）：

| from \\ event | SUBMIT_RECORD | UPDATE_CASES | SYNC | COMPLETE | DELETE |
| --- | --- | --- | --- | --- | --- |
| `NEW` | → `IN_PROGRESS`（:246 经 transition，仅 mark 触发） | 不变 | 不变 | → `COMPLETED`（:308 直写） | 不变（允许软删） |
| `IN_PROGRESS` | 不变 | 不变 | 不变 | → `COMPLETED` | 不变（允许软删） |
| `COMPLETED` | ✗ 抛 `TEST_REVIEW_FINISHED`（:222 守卫） | ✗ 抛 `TEST_REVIEW_FINISHED`（:207 守卫） | ✗ 抛（:386 守卫） | 幂等（返回 `COMPLETED`，:305 守卫放行） | 不变（允许软删） |
