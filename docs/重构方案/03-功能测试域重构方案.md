...
| 重构目标 | 设计模式 | 落点 |
| --- | --- | --- |
| 评审流转（T2） | State | `ReviewWorkflow`（上下文）+ `ReviewState`（受理/通过/驳回/重提状态集合），非法跃迁由状态机拒绝 |
| 计划生命周期（T2） | State + Template Method | `PlanLifecycle` 状态机；计划执行各阶段（准备→执行→汇总）以模板方法固化骨架，差异经策略注入 |
| 快照冻结（评审/计划） | Builder + Memento | `ReviewSnapshotBuilder`/`PlanSnapshotBuilder` 分步构建；评审快照支持回滚（Memento） |
| AI 解耦（T3） | Observer（领域事件） | `ReviewConclusionEvent`/`PlanAdjustedEvent` → AI 消费者（见 `06`），域内不反向依赖 service.ai |
| 用例树（T4） | Composite | `TestCaseNode` 复合结构（容器节点/叶子节点），统一遍历/统计 |
| 计划执行触发 | Command | `PlanExecutionCommand`（幂等执行、可重试、可追踪） |
| 域化对外暴露 | Facade | 各域仅以 `XxxService` 接口 + DTO 暴露，禁止跨域 import 实现类 |

> 护栏：状态机纯函数可单测；事件消费幂等；模式落点"命名即位置"（见总体计划 §4.4）。
