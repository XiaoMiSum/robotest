...
| 模式 | 目标落点 | 接口骨架 / 行为约定 |
| --- | --- | --- |
| Observer（领域事件） | 评审内容变更 → AI 评审任务消费者（见 `06`） | 目标是解除 `TestReviewServiceImpl → AiTaskService` 的反向直接调用（§4④）；评审 AI 结果本就落 `AiAnalysisTask` 字段、不写评审域表，事件化仅迁移"取消任务"触发点 |
...
4. **AI 解耦试点**：将 `TestReviewServiceImpl` 对 `service.ai` 的调用改为发布事件 → AI 评审任务消费者（见 `06`）；仅迁移"离开 in_progress/删除时取消 `review_check` 任务"这一反向耦合点（评审结论不写评审域表，见 D3）。
