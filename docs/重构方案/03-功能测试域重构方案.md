...
**ArchUnit 断言**（后续切片）：
- `domain/review` 中调用 `setStatus(...)` 的类仅有 `ReviewWorkflowImpl` 及其状态实现。
...
| 模式 | 目标落点 | 接口骨架 / 行为约定 |
| --- | --- | --- |
| Observer（领域事件） | 评审内容变更 → AI 评审任务消费者（见 `06`） | 目标是解除 `TestReviewServiceImpl → AiTaskService` 的反向直接调用（§4④）；评审 AI 结果本就落 `AiAnalysisTask` 字段、不写评审域表，事件化仅迁移"取消任务"触发点。事件语义（用户已确认合并）：`ReviewLifecycleEvent`（评审完成/删除 → AI 域取消 review_check）＋ `ReviewConclusionEvent` 骨架（结论写回事件，本轮只建类不接消费者，见 `06` §5.2 对齐） |
