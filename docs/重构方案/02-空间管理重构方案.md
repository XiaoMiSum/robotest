...
| 重构目标 | 设计模式 | 落点 |
| --- | --- | --- |
| C4 上下文收敛（W1） | Facade + Interceptor | `WorkspaceContextService`（唯一解析门面）+ `ContextHeaderInterceptor`（统一注入/校验上下文头），Controller/Service 禁止自行 getHeader |
| 邀请生命周期（W2） | State | `InvitationStateMachine`（上下文）+ `InvitationState`（pending/accepted/expired/revoked 状态集合），非法跃迁拒绝 |
| 成员变更联动 | Observer（事件） | `MembershipChangedEvent` → 项目访问/权限联动刷新 |
| 管理端只读（W3） | CQRS 读模型 | `AdminWorkspaceQuery` 只读查询模型；写操作留在运营端 Service |
| 上下文来源解析 | Strategy | `ContextResolverStrategy`（默认仅请求头 C4，其他来源不引入） |
| 前端邀请/列表（W4） | Hook | `useInvitation`/`useWorkspaceTable`（自定义 Hook 复用生命周期/分页逻辑） |

> 护栏：`WorkspaceContextService` 为唯一解析点；`MembershipChangedEvent` 幂等消费（见总体计划 §4.4）。
