...
| 重构目标 | 设计模式 | 落点 |
| --- | --- | --- |
| 网关统一入口（G2） | Facade | `AiGateway` 唯一会话/推理入口，屏蔽 Provider 差异 |
| Provider 切换（G2） | Strategy + Registry + Adapter | `ProviderRegistry`（工厂注册表选择/降级/超时）+ Provider 适配器（openai-compat 等），消灭 if-else 族 |
| 配置权威（G1/G2） | Singleton（唯一 ConfigService）+ Proxy | `AiConfigService` 唯一读写入口；密钥经 `SecretMaskingProxy` 回显掩码与日志脱敏（与 `07` 密文规范） |
| 任务处理（G3） | Command + Template Method | `AiTaskCommand`（幂等键/重试/补偿）+ 任务模板 Handler（提交/执行/失败处理骨架） |
| 域解耦（G1） | Observer（领域事件） | AI 作为消费者订阅 `ReviewConclusionEvent`/`BugClusteredEvent` 等，仅经 `service.ai.port` 反向调用，不改写域核心 |
| 会话上下文（G5） | Builder | `ContextBuilder` 组合角色/工具/历史为上下文；SSE 流由 `StreamingPublisher` 封装 |
| 工具链（G4/support） | Chain of Responsibility | assistant 工具按职责链（解析→校验→调用→回写）处理，可插拔 |
| 向量查询 | Strategy | `EmbeddingStrategy`/`RetrievalStrategy` 算法族切换（模型兼容/降级） |

> 护栏：业务域对 `service.ai` 实现类 import 为 0（ArchUnit 断言）；Provider 以 `ProviderRegistry` 收口选择逻辑，不改请求体协议；密钥零明文（见总体计划 §4.4）。
