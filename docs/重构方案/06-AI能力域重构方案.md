...
```java
public record ReviewLifecycleEvent(UUID reviewId) {}                 // ✓ 本轮落地：评审完成/删除 → AI 域取消 review_check
public record ReviewConclusionEvent(UUID reviewId, String verdict,
        String reason) {}                                            // 骨架：本轮只建类不接消费者
// 消费者：service.ai/review 内订阅 ReviewLifecycleEvent，调用 AiTaskService.cancelByTypeAndTarget(REVIEW_CHECK, reviewId)
// ReviewConclusionEvent 的 AiInference(port) 写回链路留给后续增量（06 §5.2 标注待办）
```
...
2. **试点解耦（与 `03` Review 切片共用）**：定义 `ReviewLifecycleEvent(reviewId)` 与 `ReviewConclusionEvent(reviewId, verdict, reason)`（合并语义，用户已确认）。本轮：`TestReviewServiceImpl` 发布 `ReviewLifecycleEvent`，AI 域消费者订阅并取消 `review_check` 任务（行为不变）；`ReviewConclusionEvent` 仅建骨架，其 `AiInference(port)` 写回链路待后续增量接线。
