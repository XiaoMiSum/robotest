...
**Review 解耦试点事件集（与 `03` §4④ 共用对齐，已确认合并语义）**：
```java
public record ReviewLifecycleEvent(UUID reviewId) {}                 // ✓ 落地：评审完成/删除 → AI 域取消 review_check
public record ReviewConclusionEvent(UUID reviewId, String verdict,
        String reason) {}                                            // ✓ 落地：completeReview 发布，AI 域生成 review 级结论（接线见 §5.2）
// 消费者：service.ai/review 内订阅 ReviewLifecycleEvent，调用 AiTaskService.cancelByTypeAndTarget(REVIEW_CHECK, reviewId)
// 消费者：service.ai/review 内订阅 ReviewConclusionEvent，入队 type=review_conclusion（见 §5.2）
```
...
2. **试点解耦（与 `03` Review 切片共用）**：定义 `ReviewLifecycleEvent(reviewId)` 与 `ReviewConclusionEvent(reviewId, verdict, reason)`（合并语义，用户已确认）。本轮：`TestReviewServiceImpl` 发布 `ReviewLifecycleEvent`，AI 域消费者订阅并取消 `review_check` 任务（行为不变）；`ReviewConclusionEvent` 写回链路按 §5.2 增量接线（自动队列 + 手动 SSE + 有界重试 + 覆盖落库 + 前端展示）。
...
## 5. 评审结论写回链路（ReviewConclusionEvent 增量接线）

> 本增量为 `03` §4④ 的 `ReviewConclusionEvent` 骨架接线：评审完成后自动生成 review 级 AI 结论，并支持手动触发、SSE 实时感知、失败有界重试与前端展示。结论仍落 `AiAnalysisTask` 字段，不写评审域表（`03` D3 不变）。

### 5.1 双路径设计

| 路径 | 触发 | 执行 | LLM 调用 |
| --- | --- | --- | --- |
| 自动 | `TestReviewServiceImpl.completeReview` 发布 `ReviewConclusionEvent` → 消费者入队 | `AiTaskService` 异步队列（`ReviewConclusionTaskHandler`，Command 分发） | `AiGatewayService.completeStructured` |
| 手动 | `POST /api/project/ai/reviews/{id}/conclusion`（SSE） | 请求线程同步（复用评审摘要 SSE 模式） | `AiGatewayService.stream` |

读取统一走 `GET /api/project/ai/reviews/{id}/conclusion`，返回最近一条 `review_conclusion` 任务（含 success 结论 / 运行中 / 失败态）。双路径共享同一统计口径、业务数据组装与覆盖式落库（`deleteSuccessExcept` 保留最新 success）。

### 5.2 ReviewConclusionEvent 写回链路

**判定与发布**：`ReviewConclusionEvent(reviewId, verdict, reason)` 由评审域 `completeReview` 同步发布；`verdict` 由纯函数 `ReviewConclusionEvaluator` 按节点 `lastMark` 统计确定性判定（不经 LLM、可审计）：

| lastMark 统计 | verdict |
| --- | --- |
| 关联用例数 = 0 | `INCONCLUSIVE`（无可评对象） |
| 存在 `FAIL` | `FAIL` |
| 存在 `PENDING`（评审完成时仍有未评节点） | `INCONCLUSIVE` |
| 其余全部已评且无 FAIL | `PASS` |

**生成与校验**：消费者校验评审存在且 `completed` 后 `createTask` 入队（`lockInProgress` 防并发双建，冲突吞掉视为已受理）；handler 读定格快照（统计 + 失败项采样），`completeStructured` 生成 `{reason, keyFindings[]}`，组装最终结果 `{verdict(事件值), reason, keyFindings}`——verdict 不取自 LLM，避免判定漂移。

**重试（有界）**：自动路径对"统计→LLM→校验→落库"完整链路重试 1 次，仍失败抛业务异常由任务框架 `markFailed`；手动路径 SSE 流异常自动重开一次流；两者均不无限重试。失败后可用既有 `POST /api/project/ai/tasks/{id}/retry` 或前端"重新生成"。

**SSE 帧**（手动路径，统一帧协议 `useAiStream` 兼容）：

| event | data | 说明 |
| --- | --- | --- |
| `statistics` | 快照统计 | 建立连接即推送（SQL 精确、不依赖 LLM），前端渲染统计卡 |
| `done` | 结论对象 `{verdict, reason, keyFindings, statistics, generatedAt}` | 生成完成并已落库 |
| `error` | `{message}` | 业务异常 / 重试失败 |

**前端展示**：评审详情页新增结论面板（发起人 + aiEnabled + 已完成可见）：成功态渲染 verdict 徽标 + reason + keyFindings + 统计卡 + 生成时间；运行中提示进度；失败态展示原因并提供重试；可持续重新生成覆盖。

**验收**：
- [ ] 评审切换到 completed 后自动生成 `review_conclusion` 任务并可读；自动/手动并发不产生双任务
- [ ] verdict 四档判定（0 用例 / FAIL / PENDING / 全过）逐格单测
- [ ] 手动 SSE：生成过程 statistics + done 帧渲染；覆盖式保留最新结论
- [ ] 失败自动重试 1 次，仍失败入 FAILED 且可手动恢复
- [ ] 前端详情页结论面板全状态（成功/运行/失败/空）可用
- [ ] 业务域对 `service.ai` 实现类 import 仍为 0；事件消费在 `service.ai/review` 内

**风险与缓解**：评审完成后用例集已禁写（`03` 状态机 COMPLETED 禁 UPDATE_CASES）；模型不可用时自动路径任务 FAILED 落库、页面可重试，不阻塞评审流程。
