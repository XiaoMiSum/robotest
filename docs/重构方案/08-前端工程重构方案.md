...
| 重构目标 | 设计模式 | 落点 |
| --- | --- | --- |
| 契约映射（F1） | Adapter（防腐层） | `types/generated/`（openapi-typescript 真相源）→ 领域展示类型，映射层显式适配、禁止改动生成层 |
| god page 拆分（F2/F3） | Hook（自定义 Hook）+ Compound Components | composables（useSceneDraft/useAiConfig/useBugForm…）承载逻辑；provide/inject 共享子组件上下文 |
| 页面内模型（F3） | Hook 迁移 | 15 个页面内 `.ts` 模型转为 composables，组件仅保留视图编写 |
| Store 边界（F3） | Observer（Pinia 单例） | store 仅保留跨页共享态；页面态下沉 composables，不做全局可变单例 |
| 列表/分页复用 | Hook + Strategy | `useTable`（分页/搜索/批量默认流程 + 定制策略），达 3 次复制阈值触发 |
| 图谱画布（F5） | 复合组件 + 插槽 | `CanvasShell` 收敛画布骨架，内容经插槽注入（达标后抽象，未达标保持复制） |
| 状态渲染族 | Strategy | `BugStatusTag`/状态映射按域策略渲染（与 `05` 协同） |
| 类型构建（工厂） | Factory | 领域类型工厂（生成空/示例对象），替代散写字面量（可选、按需） |

> 护栏：类型防疡以生成层为基准、映射层显式；不引入 Service Locator / 全局多例（见总体计划 §4.4 护栏 5）。
