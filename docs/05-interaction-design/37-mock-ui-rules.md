...
- 同路径同方法存在多条规则时按优先级顺序匹配，取第一条命中规则（见 1.3）；
...
| 错误态 | 拉取失败：错误提示 + [重试] 按钮；保存失败：抽屉内红字定位首个错误项；地址冲突（错误码 1000017202）：Toast 明确提示；编辑/调试时 Mock 已被删除（错误码 1000017201）：Toast「Mock 不存在」并刷新列表 |
