...
- **加载与错误状态**：发送/本地执行按钮 loading + 禁用，响应查看区加载中占位；导入解析/导入中弹窗内进度条且禁止关闭；校验错误弹窗内红字提示；业务错误码（1000017010/1000017011/1000017012/1000017013）以中文文案提示；并发队列满弹窗提示「并发队列已满，请稍后重试」；
