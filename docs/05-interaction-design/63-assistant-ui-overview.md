...
> 所有颜色 / 圆角 / 阴影 / 过渡令牌均来自 `docs/05-interaction-design/03-visual-design.md` 第 7 节 CSS 变量定义（实现于 `web/src/assets/styles/variables.scss`），本章不另行定义色值；图标一律使用 SVG 组件（Sparkles / Check / Close / Wrench / Clipboard），禁止 emoji（正文 ASCII 示意图中的 emoji 仅为标记）。
...
### 2.2 视觉令牌（引用 `docs/05-interaction-design/03-visual-design.md` 第 7 节）
