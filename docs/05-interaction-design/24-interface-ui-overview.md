...
- **引用保护**：被场景/Mock 引用的接口禁止删除，需先解除引用（错误码 1000017103）。
...
- **加载与错误状态**：页面/详情首次加载骨架屏，筛选/刷新/翻页时表格细进度条；保存/提交按钮 loading + 禁用；校验错误表单内红字定位首个错误项；业务错误码（1000017101/1000017102/1000017103）以中文文案 Toast 或弹窗提示；乐观锁冲突（409）提示「接口已被他人修改，请刷新后重试」；
