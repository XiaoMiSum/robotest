...
分段计数与当前名称关键词组合同步更新；计数不受当前选中分段影响。列表数据与数量统计分别调用 `GET /api/workspaces` 和 `GET /api/workspaces/counts`，并忽略过期响应。无权限用户不展示创建入口，卡片网格仍保持完整数据布局。
...
- 默认每页 20 条，可切换 20/40/100；仅当前筛选结果超过单页容量时显示分页器。
