...
- **响应**：`{ "taskId": "0198…" }`
- **约束**：同项目同时仅一个进行中任务（1000013005）；范围为当前项目全部未关闭缺陷（active/resolved/rejected）；缺陷数为 0 时直接返回 1000013012（目标对象状态不允许该 AI 操作）提示无可分析数据。

### 1.2 查询聚类结果

- **路径**：`GET /api/project/ai/bugs/clustering/latest`
- **响应**：最近一次任务（含 status/progress/result，result 结构见 2.3）；无任务返回 `null`。手动刷新 = 重新发起 1.1（新任务成功后覆盖展示）。
