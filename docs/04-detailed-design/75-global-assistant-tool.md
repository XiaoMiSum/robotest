...
- **处理**：校验 token 存在（不存在/超时/已消费返回 1000013011）、归属当前用户，且令牌 `workspaceId` 与 `X-Active-Workspace` 一致（用户切换空间后不可确认，同样返回 1000013011）→ 以当前 LoginUser 执行工具（既有 Service 方法）→ 结果作为 tool 消息落库（`tool_call_id` 取令牌中的 `toolCallId`）并回填 LLM → 流式生成最终答复（帧同 3.2）；执行结果含平台内跳转链接（如新建缺陷详情路由）。
...
- **防虚构**：system 指令强制"数据必须来自工具结果，工具查不到必须明确告知查询不到"；回复中的实体链接由工具结果携带的路由数据生成（4），不允许 LLM 自造 URL。
