...
- **请求体**：结构同 1.1 响应，其中 `apiKey` 字段为字符串：非空即更新（加密后落库），`null` 或缺省表示保持原值。
- **校验**：
  - `embedding.provider` 组内填写时须为 2.5 注册表有效键（scopes 含 embedding），无效返回 1000001001；
  - `embedding.*` 后端可保留整组为空以表示未配置；管理端表单在点击保存或连通性测试时要求供应商/地址/模型/维度/密钥齐全，校验失败不提交；已配置密钥编辑时留空表示保持原值；
  - `embedding.dimension` ∈ [1, 2000]，超限返回 1000013008（HNSW 索引上限约束）；
  - `extraParams` 必须为 JSON 对象，且不得覆盖标准参数白名单键（见 4.2），违规返回 1000001001；`provider ≠ custom` 时命中供应商独有配置项模板键的值须通过模板类型/枚举校验（2.5），违规返回 1000001001；
  - `enabled = true` 保存时须存在至少一个已启用的对话模型（1.7），否则返回 1000001001；
  - `settings` 提交完整键值集（表单收集）：逐键按 1.8 定义校验类型与取值范围（未知键、类型不符或越界返回 1000001001，`planOrder.weights` 三权重之和须为 1）；后端仅持久化与内置默认值不同的键，其余键不落库（保持缺省回退默认值语义，2.2）；
  ...
  - 管理端必填校验仅由保存/连通性测试动作触发，不在输入过程中即时阻断；校验错误在对应字段行内提示。
...
- **处理**：
  ...
  - `embedding`：对固定文本发起向量化，校验返回向量长度 === 配置维度，不一致返回 1000013008。
- **响应**：`{ "ok": true, "latencyMs": 832, "detail": "..." }`；失败返回 1000013007 及上游错误摘要。
...
- **重试**：`POST /api/admin/ai/rebuild-task/retry` — 仅最近一次任务为 `failed` / `cancelled` 时可重试（重置为 pending 重新入队，同 3.5.3 处理；`cancelled` 的 rebuild 同样意味着向量数据不完整，须提供恢复入口），**任意系统管理员可操作**（不限发起人，该任务由系统自动创建）；其余状态返回 1000013006。
...
- **删除**：`DELETE /api/admin/ai/chat-models/:id` — 逻辑删除；默认模型不可删除（返回 1000001001，需先转移默认）。
- **设为默认**：`PUT /api/admin/ai/chat-models/:id/default` — 事务内先清除原默认再置新默认（唯一默认保证，见 4.11）；停用状态的模型不可设为默认（返回 1000001001）。
- **启用/停用**：`PUT /api/admin/ai/chat-models/:id/enabled` — 请求体 `{ "enabled": false }`；默认模型不可停用（返回 1000001001）。

**校验**（新建/更新共用）：`name` / `provider` / `baseUrl` / `model` 必填；`name` 全局唯一（冲突返回 1000001001）；`provider` 为 2.5 注册表有效键（scopes 含 chat）；新建时 `apiKey` 必填，更新时非空才替换密钥；`extraParams` 白名单与独有配置项校验同 1.2。
