...
`pageContext` 可空；脑图编辑页发送时由前端上下文桥自动注入。`modelId` 可选：用户在助手输入区选择的对话模型标识（交互式功能通用约定，缺省或失效回退系统默认模型，见基础设施 1 / 4.11）；同一会话内允许逐条消息切换模型，Function Calling 循环内的多次上游调用使用同一模型。
