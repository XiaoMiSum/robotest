...
- **说明**：任务归属项目须与 `X-Active-Project` 一致，否则 1000003004；全局任务（`embedding_rebuild`）不经本组接口，见 3.3.5；`result` 常规仅在 `success` 时非空，例外：`review_check` 分批累计写入，`running` / `cancelled` 状态亦可含已产出的部分结果（见 1.2 取消保留豁免与《AI 评审与测试计划辅助详细设计说明书》2.2.1）。前端轮询间隔 2 秒，任务终态后停止。

### 1.2 取消任务

- **路径**：`POST /api/project/ai/tasks/:id/cancel`
- **约束**：仅 `pending` / `running` 可取消，且仅任务发起人可操作；其余状态返回 1000013006。
- **处理**：置 `cancelled` 并中断执行线程的后续批次（见 2）；取消后中间产物的保留策略由各任务类型定义：`review_check` 分批累计写入的已产出建议**保留可查看**（SRS 1.1「此前已产出的检查结果仍可查看」，见《AI 评审与测试计划辅助详细设计说明书》2.2.1/4.1），其余任务类型的中间产物不保留；历史已完成任务的 `success` 结果快照均不受取消影响。

### 1.3 重试任务

- **路径**：`POST /api/project/ai/tasks/:id/retry`
- **约束**：仅 `failed` 可重试，且仅任务发起人可操作；若同 type + target 已有进行中任务返回 1000013005。
...
- **创建约束**：同 `type` + `target_id`（聚类为 `type` + `project_id`，`embedding_rebuild` 为 `type` 全局唯一）同时至多一个 `pending/running` 任务，插入前 `SELECT … FOR UPDATE` 校验防并发双创建。冲突处理分两类：
  - **业务任务**（review_check / bug_clustering 等）：存在进行中同类任务时**拒绝**新建，返回 1000013005；
  - **`embedding_rebuild`**：为**覆盖式创建**——管理员再次变更 Embedding 配置时，先将进行中的旧重建任务置 `cancelled`（2 协作式取消）再建新任务，不返回 1000013005（系统自动触发，语义为"以最新配置为准"）；
