...
> 本分册由《》按功能模块拆分而来。前言、引言、数据设计与公共约定见总览分册 `02-system-management-overview.md`；原章节编号保持不变，分册-章节对照见总览分册。
...
- **处理**：写入当前登录用户为 `created_by`（见 4.9）；空间落库后立即将 `adminUserId` 绑定为空间管理员成员（创建即绑定，保证「至少保留一个空间管理员」从源头成立；先校验管理员后插库，避免半成品数据）。
