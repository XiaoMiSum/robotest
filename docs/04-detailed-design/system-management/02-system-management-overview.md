...
- 时间一律走 `utils/format.ts`：`formatDateTime(generatedAt)`、`formatDateTime(createdAt)`、图表轴标签用 `MM-dd`（由 `date` 字符串截取或 `formatShortDateTime` 派生），后端 UTC+0 → 本地时区（`docs/00-spec/10-engineering/01-frontend.md` §8）。
