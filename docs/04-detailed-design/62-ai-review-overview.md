...
本文档新增一个 AI 段错误码（已回补至基础设施 2.3 总表）：

| 错误码 | 说明 | HTTP 状态 |
| ---- | ---- | ---- |
| 1000013012 | 目标对象状态不允许该 AI 操作（评审已完成时发起检查、评审未「已完成」生成摘要、计划未关联快照发起顺序推荐；基础设施总表语义另含缺陷聚类的「项目无可分析缺陷」场景，见《缺陷智能分析与向量检索详细设计说明书》3.3.1） | 409 |

> 不复用任务状态错误码 `1000013006`——其语义限定为「**任务**不存在或任务状态不允许」，本码面向评审/计划等业务对象状态校验。其余错误沿用基础设施 2.3（1000002001 无权限、1000013005 同类任务进行中等）。
...
- 摘要生成为交互式功能：`ReviewAiSummary` 的生成/重新生成入口旁内嵌公共组件 `AiModelSelect`（对话模型选择器，基础设施 2.4 / `docs/05-interaction-design/01-readme.md` §2.8），所选 `modelId` 随 3.2.1 请求提交；其余功能固定默认模型，不展示选择器；
...
| 分册 | 文件 | 覆盖章节 |
|---|---|---|
| 总览 | `62-ai-review-overview.md` | 前言、1. 引言、2. 数据设计、2.3 错误码补充、2.4 文件与组件、2.5 交互要点、2.6 单元测试点、6. 实施说明、3. 接口详细设计 |
