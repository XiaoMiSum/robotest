...
- **ProjectListPage**：以卡片网格展示当前工作空间项目，支持活跃/已归档状态分段、独立状态数量统计、项目名称或描述搜索及最近更新排序；默认每页 20 条，滚动到底部自动追加下一页。空间无项目时显示引导创建区域，项目卡片支持进入、设为默认、编辑、归档、启封、删除，当前默认项目显示默认标识；具备 `project:create` 权限时在卡片网格末尾显示新建项目卡片。
...
| 分册 | 文件 | 覆盖章节 |
|---|---|---|
| 总览 | `14-space-management-overview.md` | 前言、1. 引言、2. 数据设计、2.2 通用约定、2.3 权限矩阵、2.4 总体布局、2.5 路由规划、2.6 状态管理（Pinia Store）、2.7 核心组件、6. 错误码定义、7. 安全设计 |
