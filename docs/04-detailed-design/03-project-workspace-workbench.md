...
页面在原有统计卡片和最近评审/计划基础上，增加页头、快捷入口和最近动态；快捷入口与统计卡片分别置于左右两个等宽父卡片，内部使用统一尺寸的 2×2 网格；不新增导出、发起执行或图表功能。
...
| 统计卡片 | 点击卡片 | 用例总数进入功能测试页；评审/计划分别进入 `/workspace/projects/functional-testing?tab=reviews` 和 `/workspace/projects/functional-testing?tab=plans` |
